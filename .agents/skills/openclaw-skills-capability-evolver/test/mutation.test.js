...
describe("isHighRiskMutationAllowed", () => {
    it("allows when rigor >= 0.6 and risk_tolerance <= 0.5", () => {
        assert.ok(
            isHighRiskMutationAllowed({ rigor: 0.8, risk_tolerance: 0.3 }),
        );
    });

    it("disallows when rigor too low", () => {
        assert.ok(
            !isHighRiskMutationAllowed({ rigor: 0.4, risk_tolerance: 0.3 }),
        );
    });

    it("disallows when risk_tolerance too high", () => {
        assert.ok(
            !isHighRiskMutationAllowed({ rigor: 0.8, risk_tolerance: 0.6 }),
        );
    });
});
