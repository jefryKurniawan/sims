...
function teardownTempEnv() {
    if (origGepAssetsDir !== undefined)
        process.env.GEP_ASSETS_DIR = origGepAssetsDir;
    ...
    if (origSkillDistiller !== undefined)
        process.env.SKILL_DISTILLER = origSkillDistiller;
...
function writeEvents(events) {
    var lines =
        events
            ...
            .join("\n") + "\n";
    fs.writeFileSync(
        path.join(process.env.GEP_ASSETS_DIR, "events.jsonl"),
        lines,
    );
}
...
describe("extractJsonFromLlmResponse", () => {
    it("extracts Gene JSON from clean response", () => {
        var text =
            '{"type":"Gene","id":"gene_distilled_test","category":"repair","signals_match":["err"],"strategy":["fix it"]}';
    ...
    it("skips non-Gene JSON objects", () => {
        var text =
            '{"type":"Capsule","id":"cap1"} then {"type":"Gene","id":"gene_distilled_y","category":"c","signals_match":["s"],"strategy":["do"]}';
        var gene = extractJsonFromLlmResponse(text);
        assert.ok(gene);
        assert.equal(gene.type, "Gene");
...
describe("validateSynthesizedGene", () => {
    ...
    it("rejects gene without signals_match", () => {
        ...
        assert.ok(
            result.errors.some(function (e) {
                return e.includes("signals_match");
            }),
        );
    });
    ...
    it("detects full overlap with existing gene", () => {
        var existing = [
            { id: "gene_existing", signals_match: ["error", "repair"] },
        ];
        var gene = {
            type: "Gene",
            id: "gene_distilled_dup",
            category: "repair",
            signals_match: ["error", "repair"],
            strategy: ["fix"],
    ...
    it("deduplicates id if conflict with existing gene", () => {
        var existing = [
            { id: "gene_distilled_conflict", signals_match: ["other"] },
        ];
        var gene = {
            type: "Gene",
            id: "gene_distilled_conflict",
            category: "opt",
            signals_match: ["different"],
            strategy: ["do"],
            constraints: { forbidden_paths: [".git", "node_modules"] },
        };
        var result = validateSynthesizedGene(gene, existing);
...
describe("analyzePatterns", () => {
    ...
    it("identifies high-frequency groups (count >= 5)", () => {
        ...
        for (var i = 0; i < 6; i++) {
            caps.push(
                makeCapsule("c" + i, "gene_a", "success", 0.9, [
                    "error",
                    "crash",
                ]),
            );
    ...
    it("detects strategy drift when summaries diverge", () => {
        var caps = [
            makeCapsule(
                ...
                0.9,
                ["err"],
                "Fixed crash in module A by patching function foo",
            ),
            makeCapsule(
                "c2",
                "gene_a",
            ...
            makeCapsule(
                ...
                0.9,
                ["err"],
                "Completely redesigned the logging infrastructure to avoid all future problems with disk IO",
            ),
        ];
        writeCapsules(caps);
        var data = collectDistillationData();
    ...
    it("identifies coverage gaps from events", () => {
        ...
        for (var i = 0; i < 5; i++) {
            events.push({
                type: "EvolutionEvent",
                signals: ["memory_leak", "performance"],
            });
        }
        writeEvents(events);
        var data = collectDistillationData();
        ...
        assert.ok(
            report.coverage_gaps.some(function (g) {
                return g.signal === "memory_leak";
            }),
        );
    });
});
...
describe("distiller state persistence", () => {
    ...
    it("writes and reads state correctly", () => {
        var state = {
            last_distillation_at: "2025-01-01T00:00:00Z",
            last_data_hash: "abc123",
