...
assert.ok(
    redactString("ghp_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx1234").includes(
        REDACTED,
    ),
    "bare ghp_ token should be redacted",
);
assert.ok(
    redactString("gho_abcdefghijklmnopqrstuvwxyz1234567890").includes(REDACTED),
    "bare gho_ token should be redacted",
);
assert.ok(
    redactString("github_pat_abcdefghijklmnopqrstuvwxyz123456").includes(
        REDACTED,
    ),
...
assert.ok(
    redactString(
        "use ghs_abcdefghijklmnopqrstuvwxyz1234567890 for auth",
    ).includes(REDACTED),
...
// AWS keys
assert.ok(
    redactString("AKIAIOSFODNN7EXAMPLE").includes(REDACTED),
    "AWS access key should be redacted",
);

// OpenAI project tokens
assert.ok(
    redactString("sk-proj-bxOCXoWsaPj0IDE1yqlXCXIkWO1f").includes(REDACTED),
    "sk-proj- token should be redacted",
);

// Anthropic tokens
assert.ok(
    redactString("sk-ant-api03-abcdefghijklmnopqrst").includes(REDACTED),
    "sk-ant- token should be redacted",
);

// npm tokens
assert.ok(
    redactString("npm_abcdefghijklmnopqrstuvwxyz1234567890").includes(REDACTED),
...
// Password fields
assert.ok(
    redactString("password=mysecretpassword123").includes(REDACTED),
    "password= should be redacted",
);
assert.ok(
    redactString('PASSWORD: "hunter2xyz"').includes(REDACTED),
    "PASSWORD: should be redacted",
);
...
assert.strictEqual(
    redactString("error: something failed"),
    "error: something failed",
);
assert.strictEqual(
    redactString("fix the bug in parser"),
