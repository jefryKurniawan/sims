    ...
    try {
        if (fs.existsSync(lockFile)) {
            ...
            if (!Number.isFinite(pid) || pid <= 0) {
                console.log(
                    "[Singleton] Corrupt lock file (invalid PID). Taking over.",
                );
            } else {
                try {
                    process.kill(pid, 0);
                    console.log(
                        `[Singleton] Evolver loop already running (PID ${pid}). Exiting.`,
                    );
                    return false;
                } catch (e) {
                    console.log(
                        `[Singleton] Stale lock found (PID ${pid}). Taking over.`,
                    );
                }
            }
        }
...
async function main() {
    ...
    if (command === "run" || command === "/evolve" || isLoop) {
        ...
        if (isLoop) {
            ...
            const maxSleepMs = parseMs(
                process.env.EVOLVER_MAX_SLEEP_MS,
                300000,
            );
            const idleThresholdMs = parseMs(
                process.env.EVOLVER_IDLE_THRESHOLD_MS,
                500,
            );
            ...
            const maxCyclesPerProcess =
                parseMs(process.env.EVOLVER_MAX_CYCLES_PER_PROCESS, 100) || 100;
            const maxRssMb =
                parseMs(process.env.EVOLVER_MAX_RSS_MB, 500) || 500;
            const suicideEnabled =
                String(process.env.EVOLVER_SUICIDE || "").toLowerCase() !==
                "false";
            ...
            try {
                ...
                console.warn(
                    "[Heartbeat] Failed to start: " + (e.message || e),
                );
            }
                ...
                try {
                    ...
                    if (!ok || dt < idleThresholdMs) {
                        currentSleepMs = Math.min(
                            maxSleepMs,
                            Math.max(minSleepMs, currentSleepMs * 2),
                        );
                    } else {
                    ...
                    if (suicideEnabled) {
                        ...
                        if (
                            cycleCount >= maxCyclesPerProcess ||
                            memMb > maxRssMb
                        ) {
                            console.log(
                                `[Daemon] Restarting self (cycles=${cycleCount}, rssMb=${memMb.toFixed(0)})`,
                            );
                            try {
                                const spawnOpts = {
                                    detached: true,
                                ...
                                console.error(
                                    "[Daemon] Spawn failed, continuing current process:",
                                    spawnErr.message,
                                );
                            }
                        }
                    ...
                    try {
                        ...
                        const lastSignals =
                            st1 &&
                            st1.last_run &&
                            Array.isArray(st1.last_run.signals)
                                ? st1.last_run.signals
                                : [];
                        if (lastSignals.includes("force_steady_state")) {
                            ...
                            lastSignals.includes("evolution_saturation")
                        ) {
                    ...
                    await sleepMs(
                        (currentSleepMs + jitter) * saturationMultiplier,
                    );
                } catch (loopErr) {
                    console.error(
                        "[Daemon] Unexpected loop error (recovering): " +
        ...
        const intentFlag = args.find(
            (a) => typeof a === "string" && a.startsWith("--intent="),
        );
        const summaryFlag = args.find(
            (a) => typeof a === "string" && a.startsWith("--summary="),
        );
        const intent = intentFlag ? intentFlag.slice("--intent=".length) : null;
        const summary = summaryFlag
            ? summaryFlag.slice("--summary=".length)
            : null;
        ...
        try {
            ...
            if (res && res.capsule)
                console.log(JSON.stringify(res.capsule, null, 2));
                ...
                try {
                    ...
                    if (shouldDistill()) {
                        ...
                        if (dr && dr.ok && dr.promptPath) {
                            console.log("\n[DISTILL_REQUEST]");
                            console.log(
                                "Distillation prompt ready. Read the prompt file, process it with your LLM,",
                            );
                            console.log(
                                "save the LLM response to a file, then run:",
                            );
                            console.log(
                                "  node index.js distill --response-file=<path_to_llm_response>",
                            );
                    ...
                    console.warn(
                        "[Distiller] Init failed (non-fatal): " +
                            (e.message || e),
                    );
                }
            }
        ...
        const responseFileFlag = args.find(
            (a) => typeof a === "string" && a.startsWith("--response-file="),
        );
        if (!responseFileFlag) {
            console.error(
        ...
        const responseFilePath = responseFileFlag.slice(
        ...
        try {
            ...
            if (result && result.ok) {
                ...
                console.warn(
                    "[Distiller] Distillation did not produce a gene: " +
        ...
        const lastSolid =
            state && state.last_solidify ? state.last_solidify : null;
        ...
        const geneId = lastRun.selected_gene_id
        ...
        if (gene) {
            ...
            console.log(
                "  (no gene selected or gene not found: " +
                    (geneId || "none") +
                    ")",
            );
        }
        ...
        if (mutation) {
            ...
            if (mutation.rationale)
                console.log("  Rationale:  " + mutation.rationale);
        } else {
        ...
        const {
            ...
            readCallLog,
            getLogPath,
        } = require("./src/gep/assetCallLog");

        const runIdFlag = args.find(
            (a) => typeof a === "string" && a.startsWith("--run="),
        );
        const actionFlag = args.find(
            (a) => typeof a === "string" && a.startsWith("--action="),
        );
        const lastFlag = args.find(
            (a) => typeof a === "string" && a.startsWith("--last="),
        );
        const sinceFlag = args.find(
            (a) => typeof a === "string" && a.startsWith("--since="),
        );
        ...
        if (lastFlag)
            opts.last = parseInt(lastFlag.slice("--last=".length), 10);
        ...
        if (jsonMode) {
            ...
            if (summary.entries.length > 0) {
                ...
                for (const e of show) {
                    ...
                    const assetShort = e.asset_id
                        ? e.asset_id.slice(0, 20) + "..."
                        : "(none)";
                    const sigPreview = Array.isArray(e.signals)
                        ? e.signals.slice(0, 3).join(", ")
                        : "";
                    console.log(
                        `    [${ts}] ${e.action || "?"}  asset=${assetShort}  score=${e.score || "-"}  mode=${e.mode || "-"}  signals=[${sigPreview}]  run=${e.run_id || "-"}`,
                    );
                }
            } else {
                console.log("\n  No entries found.");
