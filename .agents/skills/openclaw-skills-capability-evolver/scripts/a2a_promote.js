...
function main() {
    ...
    var typeRaw = String(args.kv.get("type") || "")
        .trim()
        .toLowerCase();
    var validated =
        args.flags.has("validated") ||
        String(args.kv.get("validated") || "") === "true";
    var limit = Number.isFinite(Number(args.kv.get("limit")))
        ? Number(args.kv.get("limit"))
        : 500;

    if (!id || !typeRaw)
        throw new Error(
            "Usage: node scripts/a2a_promote.js --type capsule|gene|event --id <id> --validated",
        );
    if (!validated)
        throw new Error(
            "Refusing to promote without --validated (local verification must be done first).",
        );
    ...
    for (var i = 0; i < external.length; i++) {
        if (
            external[i] &&
            external[i].type === type &&
            String(external[i].id) === id
        ) {
    ...
    if (!candidate)
        throw new Error(
            "Candidate not found in external zone: type=" + type + " id=" + id,
        );

    if (type === "Gene") {
        var validation = Array.isArray(candidate.validation)
        ...
        for (var j = 0; j < validation.length; j++) {
            ...
            if (!solidifyMod.isValidationCommandAllowed(c)) {
                throw new Error(
                    "Refusing to promote Gene " +
                        id +
                        ': validation command rejected by safety check: "' +
                        c +
    ...
    if (!promoted.schema_version)
        promoted.schema_version = contentHash.SCHEMA_VERSION;
    ...
    if (type === "EvolutionEvent") {
        ...
        if (emitDecisions) {
            try {
                var dmEv = a2aProto.buildDecision({
                    assetId: promoted.asset_id,
                    localId: id,
                    decision: "accept",
                    reason: "event promoted for provenance tracking",
                });
        ...
        if (emitDecisions) {
            try {
                var dm = a2aProto.buildDecision({
                    ...
                    localId: id,
                    decision: "accept",
                    reason: "capsule promoted after validation",
                });
    ...
    for (var k = 0; k < localGenes.length; k++) {
        if (
            localGenes[k] &&
            localGenes[k].type === "Gene" &&
            String(localGenes[k].id) === id
        ) {
            exists = true;
            break;
        }
        ...
        if (emitDecisions) {
            try {
                var dm2 = a2aProto.buildDecision({
                    ...
                    localId: id,
                    decision: "reject",
                    reason: "local gene with same ID already exists",
                });
    ...
    if (emitDecisions) {
        try {
            var dm3 = a2aProto.buildDecision({
                assetId: promoted.asset_id,
                localId: id,
                decision: "accept",
                reason: "gene promoted after safety audit",
            });
