...
function inferBumpFromSubjects(subjects) {
    ...
    return {
        bump: "patch",
        reason: "default to patch for non-breaking changes",
    };
}
...
function writePrivateSemverNote(note) {
    ...
    fs.writeFileSync(
        path.join(privateDir, "semver_suggestion.json"),
...
function writePrivateSemverPrompt(note) {
    ...
    fs.writeFileSync(
        path.join(privateDir, "semver_prompt.md"),
        prompt + "\n",
        "utf8",
    );
}
    ...
    for (const abs of all) {
        ...
        for (const pref of forbiddenPrefixes) {
            if (rel.startsWith(pref)) {
                throw new Error(
                    `Build validation failed: forbidden path in output: ${rel}`,
                );
            }
        }
    }
...
function main() {
    ...
    const releaseVersion =
        process.env.RELEASE_VERSION || semver.suggestedVersion;
    ...
    ensureDir(privateDir);
    fs.writeFileSync(
        path.join(privateDir, "public_build_info.json"),
    ...
    if (semver && semver.suggestedVersion) {
        ...
        process.stdout.write(
            `SemVer decision: ${semver.decision ? semver.decision.bump : "unknown"}\n`,
        );
    }
}
