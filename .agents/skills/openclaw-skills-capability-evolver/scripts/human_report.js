...
function generateHumanReport() {
    ...
    entries.forEach((entry) => {
        ...
        else if (lowerBody.includes("feishu-sticker"))
            component = "feishu-sticker";
        else if (lowerBody.includes("git-sync")) component = "git-sync";
        else if (
            lowerBody.includes("capability-evolver") ||
            lowerBody.includes("evolve.js")
        )
            component = "capability-evolver";
        else if (lowerBody.includes("interaction-logger"))
            component = "interaction-logger";
        else if (lowerBody.includes("chat-to-image"))
            component = "chat-to-image";
        else if (lowerBody.includes("safe_publish"))
            component = "capability-evolver";
        ...
        if (
            ...
            lowerBody.includes("harden")
        ) {
            category = "Security & Stability";
        } else if (
            lowerBody.includes("optimiz") ||
            lowerBody.includes("performance") ||
            lowerBody.includes("memory") ||
            lowerBody.includes("fast")
        ) {
        ...
        if (summaryLines.length > 0) {
            ...
            if (
                !exists &&
                !summary.includes("Stability Scan OK") &&
                !summary.includes("Workspace Sync")
            ) {
