...
function canUseClawhub() {
    ...
    if (!exe)
        return {
            ok: false,
            reason: "clawhub CLI not found (install: npm i -g clawhub)",
        };
    return { ok: true, exe };
}
...
function publishToClawhub({
    ...
    try {
        const inspect = spawnClawhub(
            ok.exe,
            ["inspect", slug, "--version", version],
            { stdio: "ignore" },
        );
        if (inspect.status === 0) {
            process.stdout.write(
                `ClawHub already has ${slug}@${version}. Skipping.\n`,
            );
            return;
        }
    } catch (e) {
    ...
    if (dryRun) {
        process.stdout.write(
            `[dry-run] ${ok.exe} ${args.map((a) => (/\s/.test(a) ? `"${a}"` : a)).join(" ")}\n`,
        );
        return;
    }
...
function ensureBranch(expected, dryRun) {
    const current =
        run("git rev-parse --abbrev-ref HEAD", { dryRun }) || expected;
...
function ensureRemote(remote, dryRun) {
    try {
        ...
        throw new Error(
            `Remote "${remote}" not found. Add it manually before running this script.`,
        );
    }
}
...
function copyDir(src, dest, dryRun) {
    ...
    if (!fs.existsSync(src))
        throw new Error(`Missing build output dir: ${src}`);
...
function createReleaseWithGh({ repo, tag, title, notes, notesFile, dryRun }) {
    ...
    if (!ghExe) {
        throw new Error(
            "gh CLI not found. Install GitHub CLI or provide a GitHub token for API-based release creation.",
        );
    }
...
function canUseGhForRelease() {
    ...
    try {
        ...
        const res = spawnSync(ghExe, ["auth", "status", "-h", "github.com"], {
            stdio: "ignore",
        });
...
function githubRequestJson({ method, repo, apiPath, token, body, dryRun }) {
    if (dryRun) {
        process.stdout.write(
            `[dry-run] GitHub API ${method} ${repo} ${apiPath}\n`,
        );
...
async function ensureReleaseWithApi({
    repo,
    tag,
    title,
    notes,
    ...
    if (!dryRun && existing.status === 200) {
        process.stdout.write(
            `Release already exists for tag ${tag}. Skipping.\n`,
        );
        return;
    }

    const bodyText =
        readReleaseNotes(notes, notesFile) ||
    ...
    if (!dryRun && (created.status < 200 || created.status >= 300)) {
        const msg =
            (created.json && created.json.message) ||
            created.raw ||
            "Unknown error";
        throw new Error(
            `Failed to create GitHub Release (${created.status}): ${msg}`,
        );
    }
...
function getContributorsSinceLastRelease() {
    ...
    try {
        ...
        const raw = execSync(`git log ${range} --pretty="%aN <%aE>"`, {
            encoding: "utf8",
            cwd: process.cwd(),
            stdio: ["ignore", "pipe", "ignore"],
        }).trim();
...
function main() {
    ...
    const useBuildOutput =
        String(process.env.PUBLIC_USE_BUILD_OUTPUT || "true").toLowerCase() ===
        "true";
    const releaseOnly =
        String(process.env.PUBLIC_RELEASE_ONLY || "").toLowerCase() === "true";

    const clawhubSkip =
        String(process.env.CLAWHUB_SKIP || "").toLowerCase() === "true";
    const clawhubPublish =
        String(process.env.CLAWHUB_PUBLISH || "").toLowerCase() === "false"
            ? false
            : !clawhubSkip;
    // Workaround for registry redirect/auth issues: default to the www endpoint.
    const clawhubRegistry =
        process.env.CLAWHUB_REGISTRY || "https://www.clawhub.ai";
    ...
    const releaseSkip =
        String(process.env.RELEASE_SKIP || "").toLowerCase() === "true";
    ...
    const releaseCreate =
        String(process.env.RELEASE_CREATE || "").toLowerCase() === "true"
            ? true
            : !releaseSkip;
    const releaseUseGh =
        String(process.env.RELEASE_USE_GH || "").toLowerCase() === "true";

    // If not provided, infer from build output package.json version.
    if (!releaseTag && useBuildOutput) {
        try {
            const builtPkg = JSON.parse(
                fs.readFileSync(
                    path.resolve(process.cwd(), outDir, "package.json"),
                    "utf8",
                ),
            );
            if (builtPkg && builtPkg.version)
                releaseTag = `v${builtPkg.version}`;
            if (!releaseTitle && releaseTag) releaseTitle = releaseTag;
        } catch (e) {}
    }

    const releaseVersion = String(releaseTag || "").startsWith("v")
        ? String(releaseTag).slice(1)
        : "";
    ...
    if (!releaseOnly) {
        if (!useBuildOutput) {
            ...
            run(`git push ${publicRemote} ${sourceBranch}:${publicBranch}`, {
                dryRun,
            });
        } else {
            ...
            run(
                `git clone --depth 1 https://github.com/${publicRepo}.git "${tmpRepoDir}"`,
                { dryRun },
            );
            run(`git -C "${tmpRepoDir}" checkout -B ${publicBranch}`, {
                dryRun,
            });

            // Replace repo contents with build output (except .git)
            if (!dryRun) {
                const entries = fs.readdirSync(tmpRepoDir, {
                    withFileTypes: true,
                });
                for (const ent of entries) {
                    if (ent.name === ".git") continue;
                    fs.rmSync(path.join(tmpRepoDir, ent.name), {
                        recursive: true,
                        force: true,
                    });
                }
            ...
            const msg = releaseTag
                ? `Release ${releaseTag}`
                : `Publish build output`;

            // If build output is identical to current public branch, skip commit/push.
            const pending = run(`git -C "${tmpRepoDir}" status --porcelain`, {
                dryRun,
            });
            if (!dryRun && !pending) {
                process.stdout.write(
                    "Public repo already matches build output. Skipping commit/push.\n",
                );
            } else {
                const contributors = getContributorsSinceLastRelease();
                let commitMsg = msg.replace(/"/g, '\\"');
                if (contributors.length > 0) {
                    const trailers = contributors
                        .map((c) => `Co-authored-by: ${c}`)
                        .join("\n");
                    commitMsg += `\n\n${trailers.replace(/"/g, '\\"')}`;
                    process.stdout.write(
                        `Including ${contributors.length} contributor(s) in publish commit.\n`,
                    );
                }
                run(
                ...
                run(`git -C "${tmpRepoDir}" push origin ${publicBranch}`, {
                    dryRun,
                });
            }
            ...
            if (releaseTag) {
                ...
                try {
                    ...
                    const exists = run(
                        `git -C "${tmpRepoDir}" tag --list ${releaseTag}`,
                        { dryRun },
                    );
                    if (!dryRun && exists) {
                        process.stdout.write(
                            `Tag ${releaseTag} already exists in public repo. Skipping tag creation.\n`,
                        );
                    } else {
                        run(
                            `git -C "${tmpRepoDir}" tag -a ${releaseTag} -m "${tagMsg.replace(/"/g, '\\"')}"`,
                            { dryRun },
                        );
                        run(
                            `git -C "${tmpRepoDir}" push origin ${releaseTag}`,
                            { dryRun },
                        );
                    }
                } catch (e) {
        ...
        if (releaseTag) {
            if (!useBuildOutput) {
                ...
                run(
                    `git tag -a ${releaseTag} -m "${msg.replace(/"/g, '\\"')}"`,
                    { dryRun },
                );
    ...
    if (clawhubPublish && releaseVersion) {
        ...
        const skillDir = useBuildOutput
            ? path.resolve(process.cwd(), outDir)
            : process.cwd();
        const changelog = releaseTitle
            ? `GitHub Release ${releaseTitle}`
