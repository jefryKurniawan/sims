...
function analyzeEvolution() {
    ...
    entries.forEach((entry) => {
        ...
        if (skillMatch) {
            ...
            // Try heuristics
            if (entry.toLowerCase().includes("feishu card"))
                skillName = "feishu-card";
            else if (entry.toLowerCase().includes("git sync"))
                skillName = "git-sync";
            else if (entry.toLowerCase().includes("logger"))
                skillName = "interaction-logger";
            else if (entry.toLowerCase().includes("evolve"))
                skillName = "capability-evolver";
        }
        ...
        const isDuplicate = skillUpdates[skillName].some((u) =>
            u.desc.includes(description.substring(0, 20)),
        );
        if (!isDuplicate) {
    ...
    sortedSkills.forEach((skill) => {
        ...
        updates.forEach((u) => {
            ...
            if (
                lowerDesc.includes("secur") ||
                lowerDesc.includes("harden") ||
                lowerDesc.includes("permission")
            )
                icon = "[security]";
            if (lowerDesc.includes("fix") || lowerDesc.includes("patch"))
                icon = "[repair]";
            if (lowerDesc.includes("creat") || lowerDesc.includes("add"))
                icon = "[add]";
