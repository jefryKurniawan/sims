        ...
        try {
            ...
            if (pkg.dependencies && Object.keys(pkg.dependencies).length > 0) {
                if (!fs.existsSync(path.join(skillPath, "node_modules"))) {
                    ...
                    try {
                        if (
                            fs.readdirSync(path.join(skillPath, "node_modules"))
                                .length === 0
                        ) {
                            issues.push(
                                "Empty node_modules (needs npm install)",
                            );
                        }
                    } catch (e) {
...
function autoHeal(skillName, issues) {
    ...
    for (var i = 0; i < issues.length; i++) {
        if (
            issues[i] === "Missing node_modules (needs npm install)" ||
            issues[i] === "Empty node_modules (needs npm install)"
        ) {
            try {
                // Remove package-lock.json if it exists to prevent conflict errors
                try {
                    fs.unlinkSync(path.join(skillPath, "package-lock.json"));
                } catch (e) {}
                ...
                console.log(
                    "[SkillsMonitor] Auto-healed " +
                        skillName +
                        ": npm install",
                );
            } catch (e) {
                console.error(
                    "[SkillsMonitor] Failed to heal " +
                        skillName +
                        ": " +
                        e.message,
                );
