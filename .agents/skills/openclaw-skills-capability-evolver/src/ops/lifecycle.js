...
function getLoopScript() {
    ...
    var wrapper = path.join(
        WORKSPACE_ROOT,
        "skills/feishu-evolver-wrapper/index.js",
    );
...
function start(options) {
    ...
    if (pids.length > 0) {
        console.log(
            "[Lifecycle] Already running (PIDs: " + pids.join(", ") + ").",
        );
    ...
    console.log(
        "[Lifecycle] Starting: node " +
            path.relative(WORKSPACE_ROOT, script) +
            " --loop",
    );
...
if (require.main === module) {
    ...
    switch (action) {
        ...
        case "log":
            var r = tailLog();
            console.log(r.content || r.error);
            break;
        case "check":
            var health = checkHealth();
            console.log(JSON.stringify(health, null, 2));
            if (!health.healthy) {
                console.log("[Lifecycle] Restarting...");
                restart();
            }
            break;
        default:
            console.log(
                "Usage: node lifecycle.js [start|stop|restart|status|log|check]",
            );
    }
}
