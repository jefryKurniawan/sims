...
function generateInnovationIdeas() {
    ...
    if (weakAreas.includes("automation")) {
        ideas.push(
            "- Automation: Create a 'meeting-prep' skill that auto-summarizes calendar context.",
        );
        ideas.push(
            "- Automation: Build a 'broken-link-checker' for documentation.",
        );
    }
    if (weakAreas.includes("data")) {
        ideas.push(
            "- Data: Implement a 'local-vector-store' for semantic search.",
        );
        ideas.push(
            "- Data: Create a 'log-analyzer' to visualize system health trends.",
        );
    }
    ...
    if (skills.length > 50) {
        ideas.push(
            "- Optimization: Identify and deprecate unused skills (e.g., redundant search tools).",
        );
        ideas.push(
