...
const LOCAL_DEVICE_ID_FILE = path.resolve(
    __dirname,
    "..",
    "..",
    ".evomap_device_id",
...
function isContainer() {
    ...
    try {
        ...
        if (/docker|kubepods|containerd|cri-o|lxc|ecs/i.test(cgroup))
            return true;
    } catch {}
    try {
...
function generateDeviceId() {
    ...
    if (containerId) {
        return crypto
            .createHash("sha256")
            .update("evomap:container:" + containerId)
            .digest("hex")
            .slice(0, 32);
    }
...
function persistDeviceId(id) {
    ...
    try {
        fs.writeFileSync(LOCAL_DEVICE_ID_FILE, id, {
            encoding: "utf8",
            mode: 0o600,
        });
        return;
    ...
    console.error(
        "[evolver] WARN: failed to persist device_id to " +
            DEVICE_ID_FILE +
            " or " +
