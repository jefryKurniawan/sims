...
function buildFiveQuestionsShape({ title, signals, evidence }) {
    ...
    const input =
        "Recent session transcript + memory snippets + user instructions";
    const output = "A safe, auditable evolution patch guided by GEP assets";
    const invariants =
        "Protocol order, small reversible patches, validation, append-only events";
    const params =
        `Signals: ${Array.isArray(signals) ? signals.join(", ") : ""}`.trim();
    const failurePoints =
...
function extractCapabilityCandidates({ recentSessionTranscript, signals }) {
    ...
    const signalCandidates = [
        ...
        { signal: "log_error", title: "Repair recurring runtime errors" },
        {
            ...
            title: "Prevent protocol drift and enforce auditable outputs",
        },
        {
        ...
        {
            signal: "session_logs_missing",
            title: "Harden session log detection and fallback behavior",
        },
        // Opportunity signals (innovation)
        {
            signal: "user_feature_request",
            title: "Implement user-requested feature",
        },
        {
            signal: "user_improvement_suggestion",
            title: "Apply user improvement suggestion",
        },
        { signal: "perf_bottleneck", title: "Resolve performance bottleneck" },
        { signal: "capability_gap", title: "Fill capability gap" },
        {
            signal: "stable_success_plateau",
            title: "Explore new strategies during stability plateau",
        },
        {
            signal: "external_opportunity",
            title: "Evaluate external A2A asset for local adoption",
        },
    ];
    ...
    for (const sc of signalCandidates) {
        if (
            !signalList.some(
                (s) => s === sc.signal || s.startsWith(sc.signal + ":"),
            )
        )
            continue;
        const evidence = `Signal present: ${sc.signal}`;
        const shape = buildFiveQuestionsShape({
            title: sc.title,
            signals,
            evidence,
        });
