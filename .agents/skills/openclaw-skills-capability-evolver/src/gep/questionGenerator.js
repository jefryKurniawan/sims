...
const QUESTION_STATE_FILE = path.join(
    getEvolutionDir(),
...
function isDuplicate(question, recentQuestions) {
    ...
    for (var i = 0; i < recentQuestions.length; i++) {
        ...
        var qWords = new Set(
            qLower.split(/\s+/).filter(function (w) {
                return w.length > 2;
            }),
        );
        var pWords = new Set(
            prev.split(/\s+/).filter(function (w) {
                return w.length > 2;
            }),
        );
...
function generateQuestions(opts) {
    ...
    if (
        signalSet.has("recurring_error") ||
        signalSet.has("high_failure_ratio")
    ) {
        var errSig = signals.find(function (s) {
            return s.startsWith("recurring_errsig");
        });
        if (errSig) {
            var errDetail = errSig
    ...
    if (
        signalSet.has("capability_gap") ||
        signalSet.has("unsupported_input_type")
    ) {
        var gapContext = "";
        var lines = transcript.split("\n");
        for (var i = 0; i < lines.length; i++) {
            if (
                /not supported|cannot|unsupported|not implemented/i.test(
                    lines[i],
                )
            ) {
        ...
        if (gapContext) {
            candidates.push({
                question:
                    "Capability gap detected in agent environment: " +
                    gapContext +
    ...
    if (
        signalSet.has("evolution_saturation") ||
        signalSet.has("force_steady_state")
    ) {
        var recentGenes = [];
    ...
    var failStreak = signals.find(function (s) {
        return s.startsWith("consecutive_failure_streak_");
    });
    if (failStreak) {
        var streakCount =
            parseInt(
                failStreak.replace("consecutive_failure_streak_", ""),
                10,
            ) || 0;
        if (streakCount >= 4) {
            var failGene = signals.find(function (s) {
                return s.startsWith("ban_gene:");
            });
            var failGeneId = failGene
                ? failGene.replace("ban_gene:", "")
                : "unknown";
            candidates.push({
                question:
                    "Agent has failed " +
                    streakCount +
                    " consecutive evolution cycles (last gene: " +
                    failGeneId +
                    "). The current approach is exhausted. What alternative strategies or environmental fixes should be tried?",
                amount: 0,
    ...
    if (
        signalSet.has("user_feature_request") ||
        signals.some(function (s) {
            return String(s).startsWith("user_feature_request:");
        })
    ) {
        var featureLines = transcript.split("\n").filter(function (l) {
            return /\b(add|implement|create|build|i want|i need|please add)\b/i.test(
                l,
            );
        });
        if (featureLines.length > 0) {
            var featureContext = featureLines[0]
                .replace(/\s+/g, " ")
                .trim()
                .slice(0, 150);
            candidates.push({
    ...
    if (signalSet.has("perf_bottleneck")) {
        var perfLines = transcript.split("\n").filter(function (l) {
            return /\b(slow|timeout|latency|bottleneck|high cpu|high memory)\b/i.test(
                l,
            );
        });
        if (perfLines.length > 0) {
            var perfContext = perfLines[0]
                .replace(/\s+/g, " ")
                .trim()
                .slice(0, 150);
            candidates.push({
    ...
    var recentQTexts = Array.isArray(state.recentQuestions)
        ? state.recentQuestions
        : [];
    var filtered = [];
    for (
        var fi = 0;
        fi < candidates.length && filtered.length < MAX_QUESTIONS_PER_CYCLE;
        fi++
    ) {
    ...
    // Update state
    var newRecentQuestions = recentQTexts.concat(
