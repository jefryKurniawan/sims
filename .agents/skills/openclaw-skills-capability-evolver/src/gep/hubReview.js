    ...
    try {
        ...
        if (keys.length > REVIEW_HISTORY_MAX_ENTRIES) {
            ...
            const toRemove = sorted.slice(
                0,
                keys.length - REVIEW_HISTORY_MAX_ENTRIES,
            );
...
function _buildReviewContent({ outcome, gene, signals, blast, sourceType }) {
    ...
    const score =
        outcome && Number.isFinite(Number(outcome.score))
...
async function submitHubReview({
    ...
    var content = _buildReviewContent({
        outcome,
        gene,
        signals,
        blast,
    ...
    var endpoint =
        hubUrl +
        "/a2a/assets/" +
        encodeURIComponent(reusedAssetId) +
        "/reviews";
    ...
    try {
        ...
        var timer = setTimeout(function () {
            controller.abort("hub_review_timeout");
        }, 10000);
        ...
        if (res.ok) {
            ...
            console.log(
                "[HubReview] Submitted review for " +
                    reusedAssetId +
                    ": rating=" +
                    rating +
                    ", outcome=" +
            ...
            logAssetCall({
                ...
                extra: {
                    rating: rating,
                    outcome_status: outcome && outcome.status,
                },
            });
        ...
        console.log(
            "[HubReview] Hub rejected review for " +
                reusedAssetId +
                ": " +
                errCode,
        );
        ...
        console.log(
            "[HubReview] Failed (non-fatal, " + reason + "): " + err.message,
        );
        logAssetCall({
            run_id: runId || null,
