...
function chooseBestKnownPersonality(statsByKey) {
    const stats =
        statsByKey && typeof statsByKey === "object" ? statsByKey : {};
    let best = null;
...
function shouldTriggerPersonalityMutation({ driftEnabled, recentEvents }) {
    ...
    const withMut = tail.filter(
        (e) => e && typeof e.mutation_id === "string" && e.mutation_id,
    );
    if (withMut.length >= 3) {
...
function savePersonalityModel(model) {
    ...
    const out = {
        version: 1,
        current: normalizePersonalityState(
            m.current || defaultPersonalityState(),
        ),
...
function selectPersonalityForRun({ driftEnabled, signals, recentEvents } = {}) {
    ...
    if (best && best.key) {
        ...
        const diffs = getParamDeltas(base, bestState).filter(
...
function updatePersonalityStats({
    ...
    if (sc != null) {
        ...
        const prev = Number.isFinite(Number(cur.avg_score))
            ? Number(cur.avg_score)
            : 0.5;
