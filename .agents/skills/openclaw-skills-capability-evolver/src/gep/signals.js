...
function analyzeRecentHistory(recentEvents) {
    ...
    return {
        ...
        recentFailureRatio:
            tail.length > 0 ? recentFailureCount / tail.length : 0,
...
function extractSignals({
    ...
    try {
        ...
        var errPatterns =
            corpus.match(
                /(?:LLM error|"error"|"status":\s*"error")[^}]{0,200}/gi,
            ) || [];
    ...
    if (featEn)
        featureRequestSnippet = featEn[0]
            .replace(/\s+/g, " ")
            .trim()
            .slice(0, 200);
    if (
    ...
    if (
        ...
        if (!featureRequestSnippet && /我想/.test(corpus)) {
            var featWantZh = corpus.match(
                /我想\s*[，,\.。、\s]*([\s\S]{0,400})/,
            );
            featureRequestSnippet = featWantZh
                ? featWantZh[1].replace(/\s+/g, " ").trim().slice(0, 200) ||
                  "功能需求"
                : "功能需求";
        }
    ...
    if (
        !featureRequestSnippet &&
        /追加|実装|作って|機能を|追加して|が欲しい|を追加|してほしい/.test(
            corpus,
        )
    ) {
    ...
    if (!errorHit) {
        ...
        if (hasImprovement) {
            signals.push(
                "user_improvement_suggestion:" + (improvementSnippet || ""),
            );
        }
    }
