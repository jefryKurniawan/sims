...
function _readCycleCount() {
    try {
        ...
        var workspacePath = path.resolve(
            __dirname,
            "..",
            "..",
            "..",
        ...
        for (var i = 0; i < candidates.length; i++) {
            if (fs.existsSync(candidates[i])) {
                ...
                return data && Number.isFinite(data.cycleCount)
                    ? data.cycleCount
                    : 0;
            }
        }
...
function resolveStrategy(opts) {
    ...
    var name = String(process.env.EVOLVE_STRATEGY || "balanced")
        .toLowerCase()
        .trim();
    ...
    var isDefault =
        !process.env.EVOLVE_STRATEGY || name === "balanced" || name === "auto";
