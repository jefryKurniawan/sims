...
function hasErrorishSignal(signals) {
    const list = Array.isArray(signals)
        ...
        : [];
    if (
        list.includes("issue_already_resolved") ||
        list.includes("openclaw_self_healed")
    )
        return false;
    if (list.includes("log_error")) return true;
    if (
        list.some(
            (s) => s.startsWith("errsig:") || s.startsWith("errsig_norm:"),
        )
    )
        return true;
    return false;
...
function hasOpportunitySignal(signals) {
    var list = Array.isArray(signals)
...
function mutationCategoryFromContext({ signals, driftEnabled }) {
    ...
    try {
        ...
        if (
            strategy &&
            typeof strategy.innovate === "number" &&
            strategy.innovate >= 0.5
        )
            return "innovate";
    } catch (_) {}
    return "optimize";
...
function isHighRiskPersonality(p) {
    ...
    const rigor =
        p && Number.isFinite(Number(p.rigor)) ? Number(p.rigor) : null;
    const riskTol =
        p && Number.isFinite(Number(p.risk_tolerance))
            ? Number(p.risk_tolerance)
            : null;
...
function isHighRiskMutationAllowed(personalityState) {
    ...
    const riskTol =
        personalityState &&
        Number.isFinite(Number(personalityState.risk_tolerance))
...
function buildMutation({
    ...
    const ts = nowTsMs();
    const category = mutationCategoryFromContext({
        signals,
...
function isValidMutation(obj) {
    ...
    if (
        !obj.category ||
        !["repair", "optimize", "innovate"].includes(String(obj.category))
    )
        return false;
    if (!Array.isArray(obj.trigger_signals)) return false;
    if (!obj.target || typeof obj.target !== "string") return false;
    if (!obj.expected_effect || typeof obj.expected_effect !== "string")
        return false;
    if (
        !obj.risk_level ||
        !["low", "medium", "high"].includes(String(obj.risk_level))
    )
        return false;
    return true;
}
...
function normalizeMutation(obj) {
    ...
    const out = {
        ...
        category: ["repair", "optimize", "innovate"].includes(
            String(m.category),
        )
            ? String(m.category)
            : "optimize",
