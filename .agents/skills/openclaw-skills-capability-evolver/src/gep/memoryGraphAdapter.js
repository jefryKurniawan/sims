...
function buildRemoteAdapter() {
    ...
    const timeoutMs =
        Number(process.env.MEMORY_GRAPH_REMOTE_TIMEOUT_MS) || 5000;

    async function remoteCall(endpoint, body) {
        if (!remoteUrl)
            throw new Error("MEMORY_GRAPH_REMOTE_URL not configured");
    ...
    return {
        ...
        getAdvice: withFallback(
            ...
            async (opts) => {
                ...
                return {
                    currentSignalKey:
                        result.currentSignalKey ||
                        localGraph.computeSignalKey(opts.signals),
                    preferredGeneId: result.preferredGeneId || null,
                    bannedGeneIds: new Set(result.bannedGeneIds || []),
                    explanation: Array.isArray(result.explanation)
                        ? result.explanation
                        : [],
                };
            },
...
function resolveAdapter() {
    const provider = (process.env.MEMORY_GRAPH_PROVIDER || "local")
        .toLowerCase()
        .trim();
    if (provider === "remote") {
