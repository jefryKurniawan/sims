...
const LOCAL_NODE_ID_FILE = path.resolve(
    __dirname,
    "..",
    "..",
    ".evomap_node_id",
...
function _persistNodeId(id) {
    ...
    try {
        fs.writeFileSync(LOCAL_NODE_ID_FILE, id, {
            encoding: "utf8",
            mode: 0o600,
        });
        return;
...
function getNodeId() {
    ...
    console.warn(
        "[a2aProtocol] A2A_NODE_ID is not set. Computing node ID from device fingerprint. " +
...
function buildHello(opts) {
    ...
    return buildMessage({
        ...
        payload: {
            ...
            capsule_count:
                typeof o.capsuleCount === "number" ? o.capsuleCount : null,
...
function buildPublishBundle(opts) {
    ...
    if (!gene || gene.type !== "Gene" || !gene.id) {
        throw new Error(
            "publishBundle: gene must be a valid Gene with type and id",
        );
    }
    if (!capsule || capsule.type !== "Capsule" || !capsule.id) {
        throw new Error(
            "publishBundle: capsule must be a valid Capsule with type and id",
        );
    }
...
function isValidProtocolMessage(msg) {
    ...
    if (!msg.message_type || !VALID_MESSAGE_TYPES.includes(msg.message_type))
        return false;
...
function fileTransportList(opts) {
    ...
    var subdir = path.join(dir, "outbox");
    if (!fs.existsSync(subdir)) return [];
    return fs.readdirSync(subdir).filter(function (f) {
        return f.endsWith(".jsonl");
    });
}
...
function _persistNodeSecret(secret) {
    try {
        ...
        fs.writeFileSync(NODE_SECRET_FILE, secret, {
            encoding: "utf8",
            mode: 0o600,
        });
    } catch {}
...
function sendHelloToHub() {
    ...
    return fetch(endpoint, {
        ...
        .then(function (data) {
            var secret =
                (data && data.payload && data.payload.node_secret) ||
...
function sendHeartbeat() {
    ...
    if (process.env.WORKER_ENABLED === "1") {
        var domains = (process.env.WORKER_DOMAINS || "")
            .split(",")
            .map(function (s) {
                return s.trim();
            })
    ...
    return fetch(endpoint, {
        ...
        .then(function (data) {
            if (
                ...
                if (backoff > _heartbeatIntervalMs) {
                    console.warn(
                        "[Heartbeat] Rate limited by hub. Next attempt in " +
                            Math.round(backoff / 1000) +
                            "s. " +
                            "Consider increasing HEARTBEAT_INTERVAL_MS to >= " +
                            (windowMs || backoff) +
                            "ms.",
                    );
            ...
            if (data && data.status === "unknown_node") {
                console.warn(
                    "[Heartbeat] Node not registered on hub. Sending hello to re-register...",
                );
                return sendHelloToHub().then(function (helloResult) {
                    if (helloResult.ok) {
                        console.log(
                            "[Heartbeat] Re-registered with hub successfully.",
                        );
                        _heartbeatConsecutiveFailures = 0;
                    } else {
                        console.warn(
                            "[Heartbeat] Re-registration failed: " +
                                (helloResult.error || "unknown"),
                        );
                    }
                    return {
                        ok: helloResult.ok,
                        response: data,
                        reregistered: helloResult.ok,
                    };
                });
            }
        ...
        .catch(function (err) {
            ...
            if (_heartbeatConsecutiveFailures === 3) {
                console.warn(
                    "[Heartbeat] 3 consecutive failures. Network issue? Last error: " +
                        err.message,
                );
            } else if (_heartbeatConsecutiveFailures === 10) {
                console.warn(
                    "[Heartbeat] 10 consecutive failures. Hub may be unreachable. (" +
                        err.message +
                        ")",
                );
...
function startHeartbeat(intervalMs) {
    if (_heartbeatRunning) return;
    _heartbeatIntervalMs =
        intervalMs || Number(process.env.HEARTBEAT_INTERVAL_MS) || 360000; // default 6min
    ...
    sendHelloToHub()
        .then(function (r) {
            if (r.ok)
                console.log(
                    "[Heartbeat] Registered with hub. Node: " + getNodeId(),
                );
            else
                console.warn(
                    "[Heartbeat] Hello failed (will retry via heartbeat): " +
                        (r.error || "unknown"),
                );
        })
        .catch(function () {})
...
function registerTransport(name, impl) {
    ...
    if (
        !impl ||
        typeof impl.send !== "function" ||
        typeof impl.receive !== "function"
    ) {
