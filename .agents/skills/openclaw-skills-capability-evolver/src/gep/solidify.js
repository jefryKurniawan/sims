...
function readOpenclawConstraintPolicy() {
    const defaults = {
        ...
        excludeExact: [
            "event.json",
            "temp_gep_output.json",
            "temp_evolution_output.json",
            "evolution_error.log",
        ],
    ...
    try {
        ...
        const pol =
            ...
            typeof obj.evolver.constraints.countedFilePolicy === "object"
                ...
                : {};
        return {
            excludePrefixes: Array.isArray(pol.excludePrefixes)
                ? pol.excludePrefixes.map(String)
                : defaults.excludePrefixes,
            excludeExact: Array.isArray(pol.excludeExact)
                ? pol.excludeExact.map(String)
                : defaults.excludeExact,
            excludeRegex: Array.isArray(pol.excludeRegex)
                ? pol.excludeRegex.map(String)
                : defaults.excludeRegex,
            includePrefixes: Array.isArray(pol.includePrefixes)
                ? pol.includePrefixes.map(String)
                : defaults.includePrefixes,
            includeExact: Array.isArray(pol.includeExact)
                ? pol.includeExact.map(String)
                : defaults.includeExact,
            includeExtensions: Array.isArray(pol.includeExtensions)
                ? pol.includeExtensions.map(String)
...
function matchAnyExact(rel, exacts) {
    const set = new Set(
        (Array.isArray(exacts) ? exacts : []).map((x) => normalizeRelPath(x)),
    );
    return set.has(rel);
}
...
function isConstraintCountedPath(relPath, policy) {
    ...
    for (const ext of Array.isArray(policy.includeExtensions)
        ? policy.includeExtensions
        : []) {
...
function computeBlastRadius({ repoRoot, baselineUntracked }) {
    ...
    const countedFiles = changedFiles.filter((f) =>
        isConstraintCountedPath(f, policy),
    );
    const ignoredFiles = changedFiles.filter(
        (f) => !isConstraintCountedPath(f, policy),
    );
    const filesCount = countedFiles.length;

    const u = tryRunCmd("git diff --numstat", {
        cwd: repoRoot,
        timeoutMs: 60000,
    });
    const c = tryRunCmd("git diff --cached --numstat", {
        cwd: repoRoot,
        timeoutMs: 60000,
    });
    ...
    if (untracked.ok) {
        ...
        const baselineSet = new Set(
            (Array.isArray(baselineUntracked) ? baselineUntracked : []).map(
                normalizeRelPath,
            ),
        );
...
function checkConstraints({ gene, blast, blastRadiusEstimate, repoRoot }) {
    ...
    if (!gene || gene.type !== "Gene")
        return { ok: true, violations, warnings, blastSeverity };
    const constraints = gene.constraints || {};
    const DEFAULT_MAX_FILES = 20;
    const maxFiles =
        Number(constraints.max_files) > 0
            ? Number(constraints.max_files)
            : DEFAULT_MAX_FILES;
    ...
    if (blastSeverity.severity === "hard_cap_breach") {
        ...
        const breakdown = analyzeBlastRadiusBreakdown(
            blast.all_changed_files || blast.changed_files || [],
        );
        console.error(`[Solidify] ${blastSeverity.message}`);
        console.error(
            `[Solidify] Top contributing directories: ${breakdown
                .map(function (d) {
                    return d.dir + " (" + d.files + ")";
                })
                .join(", ")}`,
        );
    ...
    var allowSelfModify =
        String(process.env.EVOLVE_ALLOW_SELF_MODIFY || "").toLowerCase() ===
        "true";
    for (const f of blast.all_changed_files || blast.changed_files || []) {
    ...
    if (repoRoot) {
        ...
        newSkillDirs.forEach(function (skillName) {
            ...
            try {
                var entries = fs.readdirSync(skillDir).filter(function (e) {
                    return !e.startsWith(".");
                });
                if (entries.length < 2) {
                    warnings.push(
                        "incomplete_skill: skills/" +
                            skillName +
                            "/ has only " +
                            entries.length +
                            " file(s). New skills should have at least index.js + SKILL.md.",
                    );
                }
            } catch (e) {
        ...
        var ethicsBlockPatterns = [
            {
                ...
                rule: "safety",
                msg: "ethics: strategy attempts to bypass safety mechanisms",
            },
            {
                re: /(?:keylogger|screen\s*capture|webcam\s*hijack|mic(?:rophone)?\s*record)/i,
                rule: "human_welfare",
                msg: "ethics: covert monitoring tool in strategy",
            },
            {
                re: /(?:social\s+engineering|phishing)\s+(?:attack|template|script)/i,
                rule: "human_welfare",
                msg: "ethics: social engineering content in strategy",
            },
            {
                re: /(?:exploit|hack)\s+(?:user|human|people|victim)/i,
                rule: "human_welfare",
                msg: "ethics: human exploitation in strategy",
            },
            {
                re: /(?:hide|conceal|obfuscat)\w*\s+(?:action|behavior|intent|log)/i,
                rule: "transparency",
                msg: "ethics: strategy conceals actions from audit trail",
            },
        ];
...
const BLAST_RADIUS_HARD_CAP_FILES =
    Number(process.env.EVOLVER_HARD_CAP_FILES) || 60;
const BLAST_RADIUS_HARD_CAP_LINES =
    Number(process.env.EVOLVER_HARD_CAP_LINES) || 20000;
...
function classifyBlastSeverity({ blast, maxFiles }) {
    ...
    if (
        files > BLAST_RADIUS_HARD_CAP_FILES ||
        lines > BLAST_RADIUS_HARD_CAP_LINES
    ) {
        return {
    ...
    if (!Number.isFinite(maxFiles) || maxFiles <= 0) {
        return {
            ...
            message: "no max_files constraint defined",
        };
    }
...
function runValidations(gene, opts = {}) {
    ...
    const timeoutMs = Number.isFinite(Number(opts.timeoutMs))
        ? Number(opts.timeoutMs)
        : 180000;
    const validation = Array.isArray(gene && gene.validation)
        ? gene.validation
        : [];
    const results = [];
...
function runCanaryCheck(opts) {
    ...
    const timeoutMs =
        opts && Number.isFinite(Number(opts.timeoutMs))
            ? Number(opts.timeoutMs)
            : 30000;
...
function captureDiffSnapshot(repoRoot) {
    ...
    var staged = tryRunCmd("git diff --cached", {
        cwd: repoRoot,
        timeoutMs: 30000,
    });
    ...
    if (combined.length > DIFF_SNAPSHOT_MAX_CHARS) {
        combined =
            combined.slice(0, DIFF_SNAPSHOT_MAX_CHARS) + "\n... [TRUNCATED]";
    }
    return combined || "";
}
...
function buildFailureReason(
    ...
    if (canary && !canary.ok && !canary.skipped) {
        reasons.push(
            "canary_failed: " + String(canary.err || "").slice(0, 200),
        );
    }
...
function rollbackTracked(repoRoot) {
    const mode = String(
        process.env.EVOLVER_ROLLBACK_MODE || "hard",
    ).toLowerCase();
...
function rollbackNewUntrackedFiles({ repoRoot, baselineUntracked }) {
    ...
    var sortedDirs = Array.from(dirsToCheck).sort(function (a, b) {
...
function buildSuccessReason({ gene, signals, blast, mutation, score }) {
    ...
    if (gene && gene.id) {
        ...
        parts.push(
            `Gene ${gene.id} (${category}) matched signals [${(signals || []).slice(0, 4).join(", ")}].`,
        );
    }
    ...
    if (blast) {
        parts.push(
            `Scope: ${blast.files} file(s), ${blast.lines} line(s) changed.`,
        );
    }
    ...
    if (gene && Array.isArray(gene.strategy) && gene.strategy.length > 0) {
        parts.push(
            `Strategy applied: ${gene.strategy.slice(0, 3).join("; ").slice(0, 300)}.`,
        );
    }
...
function buildCapsuleContent({
    ...
    if (gene && gene.id) {
        parts.push(
            "Gene: " + gene.id + " (" + (gene.category || "unknown") + ")",
        );
    }
    ...
    if (blast) {
        ...
        parts.push(
            "Scope: " + blast.files + " file(s), " + blast.lines + " line(s)",
        );
    ...
    if (result.length > CAPSULE_CONTENT_MAX_CHARS) {
        result =
            result.slice(0, CAPSULE_CONTENT_MAX_CHARS) + "\n... [TRUNCATED]";
    }
...
function applyEpigeneticMarks(gene, envFingerprint, outcomeStatus) {
    ...
    const platform =
        envFingerprint && envFingerprint.platform
            ? String(envFingerprint.platform)
            : "";
    const arch =
        envFingerprint && envFingerprint.arch
            ? String(envFingerprint.arch)
            : "";
    const nodeVersion =
        envFingerprint && envFingerprint.node_version
            ? String(envFingerprint.node_version)
            : "";
    const envContext =
        [platform, arch, nodeVersion].filter(Boolean).join("/") || "unknown";
...
function getEpigeneticBoost(gene, envFingerprint) {
    ...
    const mark = gene.epigenetic_marks.find(
        (m) => m && m.context === envContext,
    );
...
function solidify({
    ...
    if (!isGitRepo(repoRoot)) {
        console.error(
            "[Solidify] FATAL: Not a git repository (" + repoRoot + ").",
        );
        console.error(
            "[Solidify] Solidify requires git for rollback, diff capture, and blast radius.",
        );
        console.error(
            '[Solidify] Run "git init && git add -A && git commit -m init" first.',
        );
        return {
            ok: false,
            status: "failed",
    ...
    const geneId =
        lastRun && lastRun.selected_gene_id
            ? String(lastRun.selected_gene_id)
            : null;
    const selectedGene = geneId
        ? genes.find((g) => g && g.type === "Gene" && g.id === geneId)
        : null;
    const parentEventId =
    ...
    const mutationRaw =
        lastRun && lastRun.mutation && typeof lastRun.mutation === "object"
            ? lastRun.mutation
            : null;
    const personalityRaw =
        lastRun &&
        lastRun.personality_state &&
        typeof lastRun.personality_state === "object"
            ? lastRun.personality_state
            : null;
    const mutation =
        mutationRaw && isValidMutation(mutationRaw)
            ? normalizeMutation(mutationRaw)
            : null;
    const personalityState =
        personalityRaw && isValidPersonalityState(personalityRaw)
            ? normalizePersonalityState(personalityRaw)
            : null;
    const personalityKeyUsed = personalityState
        ? personalityKey(personalityState)
        : null;
    ...
    if (
        ...
        protocolViolations.push(
            "high_risk_mutation_not_allowed_by_personality",
        );
    }
    if (
        mutation &&
        mutation.risk_level === "high" &&
        !(lastRun && lastRun.personality_known)
    ) {
        protocolViolations.push(
            "high_risk_mutation_forbidden_under_unknown_personality",
        );
    }
    if (
        mutation &&
        ...
        protocolViolations.push(
            "forbidden_innovate_with_high_risk_personality",
        );
    }
    ...
    const blast = computeBlastRadius({
        repoRoot,
        baselineUntracked:
            lastRun && Array.isArray(lastRun.baseline_untracked)
                ? lastRun.baseline_untracked
                : [],
    });
    const blastRadiusEstimate =
        lastRun && lastRun.blast_radius_estimate
            ? lastRun.blast_radius_estimate
            : null;
    ...
    if (
        ...
        console.error(
            `[Solidify] Blast radius breakdown: ${JSON.stringify(breakdown)}`,
        );
        const estComp = compareBlastEstimate(blastRadiusEstimate, blast);
        if (estComp) {
            console.error(
                `[Solidify] Estimate comparison: estimated ${estComp.estimateFiles} files, actual ${estComp.actualFiles} files (${estComp.ratio}x)`,
            );
        }
    }
    ...
    if (destructiveViolations.length > 0) {
        ...
        console.error(
            `[Solidify] CRITICAL: Destructive changes detected: ${destructiveViolations.join("; ")}`,
        );
    }
    ...
    if (
        ...
        try {
            ...
            if (llmReviewResult && llmReviewResult.approved === false) {
                constraintCheck.violations.push(
                    "llm_review_rejected: " +
                        (llmReviewResult.summary || "no reason"),
                );
                constraintCheck.ok = false;
                console.log(
                    "[LLMReview] Change REJECTED: " +
                        (llmReviewResult.summary || ""),
                );
            } else if (llmReviewResult) {
                console.log(
                    "[LLMReview] Change approved (confidence: " +
                        (llmReviewResult.confidence || "?") +
                        ")",
                );
            }
        } catch (e) {
            console.log(
                "[LLMReview] Failed (non-fatal): " +
    ...
    const validationReport = buildValidationReport({
        ...
        commands: validation.results.map(function (r) {
            return r.cmd;
        }),
    ...
    const success =
        constraintCheck.ok && validation.ok && protocolViolations.length === 0;
    const ts = nowIso();
    ...
    const selectedCapsuleId =
        lastRun &&
        typeof lastRun.selected_capsule_id === "string" &&
        lastRun.selected_capsule_id.trim()
            ? String(lastRun.selected_capsule_id).trim()
            : null;
    const capsuleId = success ? selectedCapsuleId || buildCapsuleId(ts) : null;
    const derivedIntent =
        intent ||
        (mutation && mutation.category) ||
        (geneUsed && geneUsed.category) ||
        "repair";
    ...
    const sourceType =
        lastRun && lastRun.source_type
            ? String(lastRun.source_type)
            : "generated";
    const reusedAssetId =
        lastRun && lastRun.reused_asset_id
            ? String(lastRun.reused_asset_id)
            : null;
    const reusedChainId =
        lastRun && lastRun.reused_chain_id
            ? String(lastRun.reused_chain_id)
            : null;

    // LessonL: carry applied lesson IDs for Hub effectiveness adjustment
    const appliedLessons =
        lastRun && Array.isArray(lastRun.applied_lessons)
    ...
    const event = {
        ...
        meta: {
            ...
            blast_severity: constraintCheck.blastSeverity
                ? constraintCheck.blastSeverity.severity
                : null,
            blast_breakdown:
                !constraintCheck.ok && blast
                    ? analyzeBlastRadiusBreakdown(
                          blast.all_changed_files || blast.changed_files || [],
                      )
                    : null,
            blast_estimate_comparison: compareBlastEstimate(
                blastRadiusEstimate,
                blast,
            ),
            validation_ok: validation.ok,
            validation: validation.results.map((r) => ({
                cmd: r.cmd,
                ok: r.ok,
            })),
    ...
    if (success) {
                ...
                prevCapsule = Array.isArray(list)
                    ? list.find(
                          (c) =>
                              c &&
                              c.type === "Capsule" &&
                              String(c.id) === selectedCapsuleId,
                      )
                    : null;
            }
        } catch (e) {}
        const successReason = buildSuccessReason({
            gene: geneUsed,
            signals,
            blast,
            mutation,
        ...
        const capsuleContent = buildCapsuleContent({
            intent,
            gene: geneUsed,
            signals,
            blast,
        ...
        const capsuleStrategy =
            geneUsed &&
            Array.isArray(geneUsed.strategy) &&
            geneUsed.strategy.length > 0
                ? geneUsed.strategy
                : undefined;
        capsule = {
            type: "Capsule",
            ...
            trigger:
                prevCapsule &&
                Array.isArray(prevCapsule.trigger) &&
                prevCapsule.trigger.length
                    ? prevCapsule.trigger
                    : signals,
            gene:
            ...
            summary:
                s ||
                (prevCapsule && prevCapsule.summary
                    ? String(prevCapsule.summary)
                    : autoSummary),
    ...
    if (!dryRun && !success) {
        try {
            ...
            if (diffSnapshot) {
                var failedCapsule = {
                    ...
                    summary: geneUsed
                        ? "Failed: " +
                          geneUsed.id +
                          " on signals [" +
                          (signals.slice(0, 3).join(", ") || "none") +
                          "]"
                        : "Failed evolution on signals [" +
                          (signals.slice(0, 3).join(", ") || "none") +
                          "]",
                ...
                console.log(
                    "[Solidify] Preserved failed mutation as FailedCapsule: " +
                        failedCapsule.id,
                );
            }
        } catch (e) {
            console.log(
                "[Solidify] FailedCapsule capture error (non-fatal): " +
                    (e && e.message ? e.message : e),
            );
        }
    }
    ...
    if (!dryRun) {
        ...
        if (capsule) {
            const streak = computeCapsuleSuccessStreak({
    ...
    const runId =
        lastRun && lastRun.run_id
            ...
            : stableHash(
                  `${parentEventId || "root"}|${geneId || "none"}|${signalKey}`,
              );
    state.last_solidify = {
        run_id: runId,
        at: ts,
    ...
    if (!dryRun) {
        try {
            ...
            console.log(
                "[Narrative] Record failed (non-fatal): " +
                    (e && e.message ? e.message : e),
            );
        }
    }
    ...
    if (
        ...
        const autoPublish =
            String(process.env.EVOLVER_AUTO_PUBLISH || "true").toLowerCase() !==
            "false";
        const visibility = String(
            process.env.EVOLVER_DEFAULT_VISIBILITY || "public",
        ).toLowerCase();
        const minPublishScore =
            Number(process.env.EVOLVER_MIN_PUBLISH_SCORE) || 0.78;
        ...
        if (
            ...
            visibility === "public" &&
            sourceType !== "reused" &&
            (capsule.outcome.score || 0) >= minPublishScore
        ) {
            try {
                const {
                    buildPublishBundle,
                ...
                if (hubUrl) {
                    ...
                    if (geneUsed && geneUsed.type === "Gene" && geneUsed.id) {
                        ...
                        publishGene = {
                            ...
                            id:
                                capsule.gene ||
                                "gene_auto_" + (capsule.id || Date.now()),
                            category:
                                event && event.intent ? event.intent : "repair",
                            signals_match: Array.isArray(capsule.trigger)
                                ? capsule.trigger
                                : [],
                    ...
                    var parentRef =
                        ...
                        String(reusedAssetId).startsWith("sha256:")
                            ? reusedAssetId
                            : null;
                    if (parentRef) {
                    ...
                    sanitizedCapsule.asset_id =
                        computeAssetId(sanitizedCapsule);

                    var sanitizedEvent =
                        event && event.type === "EvolutionEvent"
                            ? sanitizePayload(event)
                            : null;
                    if (sanitizedEvent)
                        sanitizedEvent.asset_id =
                            computeAssetId(sanitizedEvent);

                    var publishChainId = reusedChainId || null;

                    var evolverModelName = (
                    ...
                    if (result && typeof result.then === "function") {
                        result
                            .then(function (res) {
                                if (res && res.ok) {
                                    console.log(
                                        "[AutoPublish] Published bundle (Gene+Capsule) " +
                                            (capsule.asset_id || capsule.id) +
                                            " to Hub.",
                                    );
                                } else {
                                    console.log(
                                        "[AutoPublish] Hub rejected: " +
                                            JSON.stringify(res),
                                    );
                                }
                            })
                    ...
                    logAssetCall({
                        run_id:
                            lastRun && lastRun.run_id ? lastRun.run_id : null,
                        action: "asset_publish",
                        asset_id: capsule.asset_id || capsule.id,
                        asset_type: "Capsule",
                        source_node_id: null,
                        chain_id: publishChainId || null,
                        signals: Array.isArray(capsule.trigger)
                            ? capsule.trigger
                            : [],
                        extra: {
                            source_type: sourceType,
            ...
            const reason = !autoPublish
                ? "auto_publish_disabled"
                : visibility !== "public"
                  ? "visibility_private"
                  : sourceType === "reused"
                    ? "skip_direct_reused_asset"
                    : "below_min_score";
    ...
    if (!dryRun && !success) {
        var publishAntiPatterns =
            String(
                process.env.EVOLVER_PUBLISH_ANTI_PATTERNS || "",
            ).toLowerCase() === "true";
        ...
        if (publishAntiPatterns && hubUrl && hasHighInfoFailure) {
            try {
                var {
                    buildPublishBundle: buildApBundle,
                    httpTransportSend: httpApSend,
                } = require("./a2aProtocol");
                var { sanitizePayload: sanitizeAp } = require("./sanitize");
                var apGene =
                    geneUsed && geneUsed.type === "Gene" && geneUsed.id
                        ? sanitizeAp(geneUsed)
                        : {
                              type: "Gene",
                              id: "gene_unknown_" + Date.now(),
                ...
                var apCapsule = {
                    ...
                    summary:
                        "Anti-pattern: " +
                        String(apGene.failure_reason).slice(0, 200),
                    confidence: 0,
                    blast_radius: { files: blast.files, lines: blast.lines },
                    outcome: { status: "failed", score: score },
                ...
                var apModelName = (process.env.EVOLVER_MODEL_NAME || "")
                    .trim()
                    .slice(0, 100);
                var apMsg = buildApBundle({
                    gene: apGene,
                    capsule: sanitizeAp(apCapsule),
                    event: null,
                    modelName: apModelName || undefined,
                });
                ...
                if (apResult && typeof apResult.then === "function") {
                    apResult
                        .then(function (res) {
                            if (res && res.ok)
                                console.log(
                                    "[AntiPatternPublish] Published failed bundle to Hub: " +
                                        apCapsule.id,
                                );
                            else
                                console.log(
                                    "[AntiPatternPublish] Hub rejected: " +
                                        JSON.stringify(res),
                                );
                        })
                        .catch(function (err) {
                            console.log(
                                "[AntiPatternPublish] Failed (non-fatal): " +
                                    err.message,
                            );
                        });
                }
                antiPatternPublishResult = {
                    attempted: true,
                    asset_id: apCapsule.asset_id,
                };
            } catch (e) {
                console.log(
                    "[AntiPatternPublish] Error (non-fatal): " + e.message,
                );
                antiPatternPublishResult = {
                    attempted: false,
                    reason: e.message,
    ...
    if (!dryRun && !success && event && event.outcome) {
        var failureContent = buildFailureReason(
            constraintCheck,
            validation,
            protocolViolations,
            canary,
        ...
        if (resultAssetId) {
            ...
            if (workerAssignmentId) {
                ...
                try {
                    ...
                    console.log(
                        `[WorkerComplete] Completing worker assignment "${workerAssignmentId}" with asset ${resultAssetId}`,
                    );
                    const completed = completeWorkerTask(
                        workerAssignmentId,
                        resultAssetId,
                    );
                    if (completed && typeof completed.then === "function") {
                        completed
                            .then(function (ok) {
                                if (ok) {
                                    console.log(
                                        "[WorkerComplete] Worker task completed successfully on Hub.",
                                    );
                                } else {
                                    console.log(
                                        "[WorkerComplete] Hub rejected worker completion (non-fatal).",
                                    );
                                }
                            })
                            .catch(function (err) {
                                console.log(
                                    "[WorkerComplete] Failed (non-fatal): " +
                                        (err && err.message
                                            ? err.message
                                            : err),
                                );
                    ...
                    taskCompleteResult = {
                        attempted: true,
                        task_id: lastRun.active_task_id,
                    ...
                    console.log(
                        "[WorkerComplete] Error (non-fatal): " + e.message,
                    );
                    taskCompleteResult = {
                        attempted: false,
                        reason: e.message,
                ...
                try {
                    ...
                    console.log(
                        `[TaskComplete] Completing task "${lastRun.active_task_title || taskId}" with asset ${resultAssetId}`,
                    );
                    ...
                    if (completed && typeof completed.then === "function") {
                        completed
                            .then(function (ok) {
                                if (ok) {
                                    console.log(
                                        "[TaskComplete] Task completed successfully on Hub.",
                                    );
                                } else {
                                    console.log(
                                        "[TaskComplete] Hub rejected task completion (non-fatal).",
                                    );
                                }
                            })
                            .catch(function (err) {
                                console.log(
                                    "[TaskComplete] Failed (non-fatal): " +
                                        (err && err.message
                                            ? err.message
                                            : err),
                                );
                    ...
                    console.log(
                        "[TaskComplete] Error (non-fatal): " + e.message,
                    );
                    taskCompleteResult = {
                        attempted: false,
                        reason: e.message,
    ...
    if (
        !dryRun &&
        reusedAssetId &&
        (sourceType === "reused" || sourceType === "reference")
    ) {
        try {
            ...
            if (reviewPromise && typeof reviewPromise.then === "function") {
                reviewPromise
                    .then(function (r) {
                        ...
                        if (r && r.submitted) {
                            console.log(
                                "[HubReview] Review submitted successfully (rating=" +
                                    r.rating +
                                    ").",
                            );
                        }
