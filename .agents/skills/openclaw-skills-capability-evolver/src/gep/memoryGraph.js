...
function memoryGraphPath() {
    ...
    return (
        process.env.MEMORY_GRAPH_PATH || path.join(evoDir, "memory_graph.jsonl")
    );
}
...
function aggregateEdges(events) {
    ...
    for (const ev of events) {
        ...
        const signalKey =
            ev.signal && ev.signal.key ? String(ev.signal.key) : "(none)";
        ...
        const status =
            ev.outcome && ev.outcome.status
                ? String(ev.outcome.status)
                : "unknown";
...
function edgeExpectedSuccess(edge, opts) {
    ...
    const halfLifeDays =
        opts && Number.isFinite(Number(opts.half_life_days))
            ? Number(opts.half_life_days)
            : 30;
...
function getMemoryAdvice({ signals, genes, driftEnabled }) {
    ...
    for (const ev of events) {
        ...
        const sigs =
            ev.signal && Array.isArray(ev.signal.signals)
                ? ev.signal.signals
                : [];
    ...
    for (const [geneId, info] of byGene.entries()) {
        const combined =
            info.best > 0 ? info.best + info.prior * 0.12 : info.prior * 0.4;
        scoredGeneIds.push({
            geneId,
            score: combined,
    ...
    const preferredGeneId = scoredGeneIds.length
        ? scoredGeneIds[0].geneId
        : null;
    ...
    if (bannedGeneIds.size)
        explanation.push(
            `memory_ban:${Array.from(bannedGeneIds).slice(0, 6).join(",")}`,
        );
    if (preferredGeneId) {
        const top = scoredGeneIds.find(
            (x) => x && x.geneId === preferredGeneId,
        );
...
function buildHypothesisText({
    ...
    const g = geneId
        ? `${geneId}${geneCategory ? `(${geneCategory})` : ""}`
        : "(none)";
...
function recordHypothesis({
    ...
    const signalKey = computeSignalKey(signals);
    const geneId =
        selectedGene && selectedGene.id ? String(selectedGene.id) : null;
    const geneCategory =
        selectedGene && selectedGene.category
            ? String(selectedGene.category)
            : null;
    const ts = nowIso();
    const errsig = extractErrorSignatureFromSignals(signals);
    ...
    const mutNorm =
        mutation && isValidMutation(mutation)
    ...
    const ev = {
        ...
        capsules: {
            used: Array.isArray(capsulesUsed)
                ? capsulesUsed.map(String).filter(Boolean)
                : [],
        },
        observed:
            observations && typeof observations === "object"
...
function recordAttempt({
    ...
    state.last_action = {
        ...
        capsules_used: Array.isArray(capsulesUsed)
            ? capsulesUsed.map(String).filter(Boolean)
            : [],
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
        ...
        try {
            ...
            const score = Number.isFinite(Number(o.score))
                ? clamp01(Number(o.score))
                : null;
            if (!status && score == null) continue;
            return {
                status:
                    status ||
                    (score != null && score >= 0.5 ? "success" : "failed"),
                score:
                    score != null ? score : status === "success" ? 0.75 : 0.25,
...
function inferOutcomeEnhanced({
    ...
    const prevErrCount =
        baselineObserved &&
        Number.isFinite(Number(baselineObserved.recent_error_count))
            ? Number(baselineObserved.recent_error_count)
            : null;
    const curErrCount =
        currentObserved &&
        Number.isFinite(Number(currentObserved.recent_error_count))
    ...
    return {
        status: base.status,
        score: clamp01(score),
        note: `${base.note}|heuristic_delta`,
    };
}
    ...
    const ev = {
        ...
        personality:
            last.personality_key || last.personality_state
                ...
                : null,
        gene: {
            id: last.gene_id || null,
            category: last.gene_category || null,
        },
        action: { id: String(last.action_id) },
        hypothesis: last.hypothesis_id
            ? { id: String(last.hypothesis_id) }
            : null,
        outcome: {
            ...
            note: inferred.note,
            observed: {
                current_signals: Array.isArray(signals) ? signals : [],
            },
        },
        confidence: {
...
function recordExternalCandidate({ asset, source, signals }) {
    ...
    const ev = {
        ...
        candidate: {
            ...
            trigger:
                type === "Capsule" && Array.isArray(a.trigger) ? a.trigger : [],
            gene: type === "Capsule" && a.gene ? String(a.gene) : null,
            confidence:
                type === "Capsule" && Number.isFinite(Number(a.confidence))
                    ? Number(a.confidence)
                    : null,
        },
    };
