...
function buildReusePrompt({ capsule, signals, nowIso }) {
    ...
    const trigger = Array.isArray(payload.trigger || capsule.trigger_text)
        ? (
              payload.trigger || String(capsule.trigger_text || "").split(",")
          ).join(", ")
        : "";
...
function buildAntiPatternZone(failedCapsules, signals) {
    ...
    var lines = matched.map(function (fc, idx) {
        var diffPreview = fc.diff_snapshot
            ? String(fc.diff_snapshot).slice(0, 500)
            : "(no diff)";
        return [
            "  " +
                (idx + 1) +
                ...
                (fc.gene || "unknown") +
                " | Signals: [" +
                (fc.trigger || []).slice(0, 4).join(", ") +
                "]",
            "     Failure: " +
                String(fc.failure_reason || "unknown").slice(0, 300),
...
function buildLessonsBlock(hubLessons, signals) {
    ...
    var sigSet = new Set(
        (Array.isArray(signals) ? signals : []).map(function (s) {
            return String(s).toLowerCase();
        }),
    );
    ...
    for (
        var i = 0;
        i < hubLessons.length && positive.length + negative.length < 6;
        i++
    ) {
        var l = hubLessons[i];
        ...
        var entry =
            "  - [" +
            (l.scenario || l.lesson_type || "?") +
            "] " +
            String(l.content).slice(0, 300);
        if (l.source_node_id)
            entry += " (from: " + String(l.source_node_id).slice(0, 20) + ")";
...
function buildGepPrompt({
    ...
    const selectedGeneId =
        selectedGene && selectedGene.id ? selectedGene.id : "gene_<name>";
    ...
    if (
        ...
        Array.isArray(selectedGene.strategy)
    ) {
        strategyBlock = `
...
`.trim();
    ...
    const schemaSection = SCHEMA_DEFINITIONS.replace(
        "<parent_evt_id|null>",
        parentValue,
    );
    ...
    if (uniqueSignals.length > 50) {
        optimizedSignals.push(
            `...[TRUNCATED ${uniqueSignals.length - 50} SIGNALS]...`,
        );
    }
...
`;
    ...
    if (
        uniqueSignals.includes("evolution_stagnation_detected") ||
...
`.trim();
    ...
    if (executionContextIndex > -1) {
        ...
        const allowedExecutionLength = Math.min(
            EXEC_CONTEXT_CAP,
