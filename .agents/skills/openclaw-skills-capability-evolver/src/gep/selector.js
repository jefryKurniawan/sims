function matchPatternToSignals(pattern, signals) {
    ...
    const regexLike =
        p.length >= 2 && p.startsWith("/") && p.lastIndexOf("/") > 0;
    if (regexLike) {
...
function computeDriftIntensity(opts) {
    ...
    var effectivePopulationSize =
        opts && Number.isFinite(Number(opts.effectivePopulationSize))
            ? Number(opts.effectivePopulationSize)
            : null;

    // If no Ne provided, fall back to gene pool size
    var genePoolSize =
        opts && Number.isFinite(Number(opts.genePoolSize))
...
function selectGene(genes, signals, opts) {
    ...
    const bannedGeneIds =
        opts && opts.bannedGeneIds ? opts.bannedGeneIds : new Set();
    const driftEnabled = !!(opts && opts.driftEnabled);
    const preferredGeneId =
        opts && typeof opts.preferredGeneId === "string"
            ? opts.preferredGeneId
            : null;
    ...
    const scored = genesList
        .map((g) => {
            ...
            if (s > 0 && g.id && String(g.id).startsWith(DISTILLED_PREFIX))
                s *= DISTILLED_SCORE_FACTOR;
    ...
    const filtered = useDrift
        ? scored
        : scored.filter((x) => x.gene && !bannedGeneIds.has(x.gene.id));
    if (filtered.length === 0)
        return {
            selected: null,
            alternatives: scored.slice(0, 4).map((x) => x.gene),
            driftIntensity: driftIntensity,
        };
...
function banGenesFromFailedCapsules(failedCapsules, signals, existingBans) {
    ...
    if (!Array.isArray(failedCapsules) || failedCapsules.length === 0)
        return bans;
    var geneFailCounts = {};
...
function selectGeneAndCapsule({
    ...
    const bannedGeneIds =
        memoryAdvice && memoryAdvice.bannedGeneIds instanceof Set
    ...
    const { selected, alternatives, driftIntensity } = selectGene(
        genes,
        signals,
        {
...
function buildSelectorDecision({
    ...
    if (signals && signals.length)
        reason.push(`signals: ${signals.join(", ")}`);

    if (
        memoryAdvice &&
        Array.isArray(memoryAdvice.explanation) &&
        memoryAdvice.explanation.length
    ) {
