...
function getReuseMode() {
    const m = String(
        process.env.EVOLVER_REUSE_MODE || DEFAULT_REUSE_MODE,
    ).toLowerCase();
...
function scoreHubResult(asset) {
    ...
    const streak = Math.min(
        Math.max(Number(asset.success_streak) || 0, 1),
        MAX_STREAK_CAP,
    );
...
async function hubSearch(signals, opts) {
    ...
    const threshold =
        opts && Number.isFinite(opts.threshold)
            ? opts.threshold
            : getMinReuseScore();
    const timeout =
        opts && Number.isFinite(opts.timeoutMs) ? opts.timeoutMs : 8000;
    ...
    try {
        ...
        const results =
            data && data.payload && Array.isArray(data.payload.results)
        ...
        if (selectedAssetId) {
            try {
                ...
                const timer2 = setTimeout(
                    () => controller2.abort(TIMEOUT_REASON),
                    timeout,
                );
                ...
                if (res2.ok) {
                    ...
                    const fullResults =
                        data2 &&
                        data2.payload &&
                        Array.isArray(data2.payload.results)
                ...
                console.log(
                    `[HubSearch] Phase 2 fetch failed (non-fatal): ${fetchErr.message}`,
                );
            }
        }

        console.log(
            `[HubSearch] Hit via search+fetch: ${pick.match.asset_id || "unknown"} (score=${pick.score}, mode=${pick.mode})`,
        );
        ...
        const isTimeout =
            err.name === "AbortError" ||
            (err.cause && err.cause === TIMEOUT_REASON);
        const reason = isTimeout ? "timeout" : "fetch_error";
        console.log(
            `[HubSearch] Failed (non-fatal, ${reason}): ${err.message}`,
        );
        logAssetCall({
            run_id: (opts && opts.run_id) || null,
            action: "hub_search_miss",
