...
function shouldReflect({ cycleCount, recentEvents }) {
    if (!Number.isFinite(cycleCount) || cycleCount < REFLECTION_INTERVAL_CYCLES)
        return false;
    ...
    try {
        if (fs.existsSync(logPath)) {
            ...
            if (Date.now() - stat.mtimeMs < REFLECTION_COOLDOWN_MS)
                return false;
        }
    } catch (_) {}
...
function buildReflectionContext({
    ...
    if (Array.isArray(recentEvents) && recentEvents.length > 0) {
        ...
        const successCount = last10.filter(
            (e) => e && e.outcome && e.outcome.status === "success",
        ).length;
        const failCount = last10.filter(
            (e) => e && e.outcome && e.outcome.status === "failed",
        ).length;
        const intents = {};
        last10.forEach((e) => {
        ...
        last10.forEach((e) => {
            const g =
                e && Array.isArray(e.genes_used) && e.genes_used[0]
                    ? e.genes_used[0]
                    : "unknown";
            genes[g] = (genes[g] || 0) + 1;
        });
    ...
    if (memoryAdvice) {
        ...
        if (
            Array.isArray(memoryAdvice.bannedGeneIds) &&
            memoryAdvice.bannedGeneIds.length > 0
        ) {
            parts.push(
                `- Banned genes: ${memoryAdvice.bannedGeneIds.join(", ")}`,
            );
        }
    ...
    parts.push("");
    parts.push(
        'Respond with a JSON object: { "insights": [...], "strategy_adjustment": "...", "priority_signals": [...] }',
    );
