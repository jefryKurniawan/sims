...
function getConfig() {
    ...
    return {
        ...
        cooldownMs:
            Number(process.env.EVOLVER_ISSUE_COOLDOWN_MS) ||
            DEFAULT_COOLDOWN_MS,
        minStreak:
...
function extractErrorSignature(signals) {
    ...
    var banned = signals.find(function (s) {
        return s.startsWith("ban_gene:");
    });
    if (banned)
        return (
            "Repeated failures with gene: " + banned.replace("ban_gene:", "")
        );
...
function formatRecentEvents(events) {
    ...
    var failed = events.filter(function (e) {
...
function shouldReport(signals, config) {
    ...
    if (state.lastReportedAt) {
        ...
        if (elapsed < config.cooldownMs) {
            var recentKeys = Array.isArray(state.recentIssueKeys)
