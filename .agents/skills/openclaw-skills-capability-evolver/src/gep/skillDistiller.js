...
var DISTILLER_MIN_CAPSULES =
    parseInt(process.env.DISTILLER_MIN_CAPSULES || "10", 10) || 10;
var DISTILLER_INTERVAL_HOURS =
    parseInt(process.env.DISTILLER_INTERVAL_HOURS || "24", 10) || 24;
var DISTILLER_MIN_SUCCESS_RATE =
    parseFloat(process.env.DISTILLER_MIN_SUCCESS_RATE || "0.7") || 0.7;
...
function collectDistillationData() {
    ...
    var capsulesJson = readJsonIfExists(path.join(assetsDir, "capsules.json"), {
        capsules: [],
    });
    var capsulesJsonl = readJsonlIfExists(
        path.join(assetsDir, "capsules.jsonl"),
    );
    var allCapsules = [].concat(capsulesJson.capsules || [], capsulesJsonl);

    var unique = new Map();
    allCapsules.forEach(function (c) {
    ...
    var successCapsules = allCapsules.filter(function (c) {
        ...
        var status =
            typeof c.outcome === "string" ? c.outcome : c.outcome.status;
        if (status !== "success") return false;
        var score =
            c.outcome && Number.isFinite(Number(c.outcome.score))
                ? Number(c.outcome.score)
                : 1;
...
function analyzePatterns(data) {
    ...
    var report = {
        ...
        success_rate:
            data.allCapsules.length > 0
                ? data.successCapsules.length / data.allCapsules.length
                : 0,
    };
    ...
    Object.keys(grouped).forEach(function (geneId) {
        ...
        if (g.total_count >= 5) {
            ...
            var top = Object.keys(freq)
                ...
                .slice(0, 5);
            report.high_frequency.push({
                gene_id: geneId,
                count: g.total_count,
                avg_score: Math.round(g.avg_score * 100) / 100,
                top_triggers: top,
            });
        }
        ...
        if (g.summaries.length >= 3) {
            ...
            if (first !== last) {
                ...
                if (sim < 0.6) {
                    report.strategy_drift.push({
                        gene_id: geneId,
                        similarity: Math.round(sim * 100) / 100,
                        early_summary: first.slice(0, 120),
                        recent_summary: last.slice(0, 120),
                    });
                }
            }
        }
    ...
    var gaps = Object.keys(signalFreq)
        .filter(function (s) {
            return signalFreq[s] >= 3 && !covered.has(s);
        })
        .sort(function (a, b) {
            return signalFreq[b] - signalFreq[a];
        })
        .slice(0, 10);
    if (gaps.length > 0) {
        report.coverage_gaps = gaps.map(function (s) {
            return { signal: s, frequency: signalFreq[s] };
        });
    }
        ...
        if (ch === "{") {
            ...
            if (depth === 0 && buffer.length > 2) {
                try {
                    ...
                    if (obj && typeof obj === "object" && obj.type === "Gene")
                        return obj;
                } catch (e) {}
                buffer = "";
            }
...
function buildDistillationPrompt(analysis, existingGenes, sampleCapsules) {
    ...
    return [
        ...
        '- The Gene MUST have a unique id starting with "' +
            DISTILLED_ID_PREFIX +
            '"',
...
function validateSynthesizedGene(gene, existingGenes) {
    ...
    if (gene.type !== "Gene")
        errors.push('missing or wrong type (must be "Gene")');
    ...
    if (!gene.constraints || typeof gene.constraints !== "object")
        gene.constraints = {};
    if (
        !Array.isArray(gene.constraints.forbidden_paths) ||
        gene.constraints.forbidden_paths.length === 0
    ) {
        gene.constraints.forbidden_paths = [".git", "node_modules"];
    }
    if (
        !gene.constraints.forbidden_paths.some(function (p) {
            return p === ".git" || p === "node_modules";
        })
    ) {
        errors.push(
            "constraints.forbidden_paths must include .git or node_modules",
        );
    }
    if (
    ...
    if (gene.signals_match && existingGenes && existingGenes.length > 0) {
        var newSet = new Set(
            gene.signals_match.map(function (s) {
        ...
        for (var i = 0; i < existingGenes.length; i++) {
            ...
            var egSet = new Set(
                (eg.signals_match || []).map(function (s) {
                    return String(s).toLowerCase();
                }),
            );
            if (newSet.size > 0 && egSet.size > 0) {
                ...
                newSet.forEach(function (s) {
                    if (egSet.has(s)) overlap++;
                });
                if (overlap === newSet.size && overlap === egSet.size) {
                    errors.push(
                        "signals_match fully overlaps with existing gene: " +
                            eg.id,
                    );
                }
            }
...
function shouldDistill() {
    if (String(process.env.SKILL_DISTILLER || "true").toLowerCase() === "false")
        return false;

    var state = readDistillerState();
    if (state.last_distillation_at) {
        var elapsed =
            Date.now() - new Date(state.last_distillation_at).getTime();
    ...
    var recentSuccess = recent.filter(function (c) {
        return (
            c &&
            c.outcome &&
            (c.outcome.status === "success" || c.outcome === "success")
        );
    }).length;
...
function prepareDistillation() {
    ...
    console.log(
        "[Distiller] Collected " +
            data.successCapsules.length +
            " successful capsules across " +
    ...
    if (data.successCapsules.length < DISTILLER_MIN_CAPSULES) {
        console.log(
            "[Distiller] Not enough successful capsules (" +
                data.successCapsules.length +
                " < " +
    ...
    if (state.last_data_hash === data.dataHash) {
        console.log(
            "[Distiller] Data unchanged since last distillation (hash: " +
                data.dataHash +
                "). Skipping.",
        );
    ...
    console.log(
        "[Distiller] Analysis: high_freq=" +
            analysis.high_frequency.length +
            " drift=" +
            analysis.strategy_drift.length +
            " gaps=" +
            analysis.coverage_gaps.length,
    );

    var assetsDir = paths.getGepAssetsDir();
    var existingGenesJson = readJsonIfExists(
        path.join(assetsDir, "genes.json"),
        { genes: [] },
    );
    var existingGenes = existingGenesJson.genes || [];

    var prompt = buildDistillationPrompt(
        analysis,
        existingGenes,
        data.successCapsules,
    );
...
function completeDistillation(responseText) {
    ...
    if (!rawGene) {
        ...
        console.error(
            "[Distiller] LLM response did not contain a valid Gene JSON.",
        );
    ...
    if (!validation.valid) {
        ...
        console.warn(
            "[Distiller] Gene failed validation: " +
    ...
    try {
        if (request.prompt_path) fs.unlinkSync(request.prompt_path);
    } catch (e) {}
