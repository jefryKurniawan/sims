    ...
    return {
        ...
        genes: [
            ...
            },
            {
                ...
                validation: [
                    buildValidationCmd(["src/evolve", "src/gep/prompt"]),
                ],
            },
        ],
    };
            ...
            raw.split("\n").forEach((line) => {
                if (line.trim()) {
                    try {
                        ...
                        if (parsed && parsed.type === "Gene")
                            jsonlGenes.push(parsed);
                    } catch (e) {}
                }
            });
        }
...
function upsertCapsule(capsuleObj) {
    ...
    const idx = capsules.findIndex(
        (c) =>
            c && c.type === "Capsule" && String(c.id) === String(capsuleObj.id),
    );
...
function appendFailedCapsule(capsuleObj) {
    ...
    ensureSchemaFields(capsuleObj);
    var current = readJsonIfExists(
        failedCapsulesPath(),
        getDefaultFailedCapsules(),
    );
    var list = Array.isArray(current.failed_capsules)
...
function ensureAssetFiles() {
    ...
    const files = [
        ...
        },
        {
            path: capsulesPath(),
            defaultContent:
                JSON.stringify(getDefaultCapsules(), null, 2) + "\n",
        },
    ...
    for (const f of files) {
        if (!fs.existsSync(f.path)) {
            try {
                ...
                console.error(
                    `[AssetStore] Failed to create ${f.path}: ${e.message}`,
                );
            }
        }
    }
