...
const AGENT_SESSIONS_DIR = path.join(
    os.homedir(),
    `.openclaw/agents/${AGENT_NAME}/sessions`,
);
const TODAY_LOG = path.join(
    MEMORY_DIR,
        ...
        try {
            ...
            if (data.type === "message" && data.message) {
                ...
                if (data.message.errorMessage) {
                    const errMsg =
                        typeof data.message.errorMessage === "string"
                ...
                if (
                    ...
                    (resContent.includes("success") ||
                        resContent.includes("done"))
                )
                    continue;
...
function readRealSessionLog() {
    try {
        ...
        let nonEvolverFiles = files.filter(
            (f) => !f.name.startsWith("evolver_hand_"),
        );
        ...
        if (sessionScope && nonEvolverFiles.length > 0) {
            ...
            if (scopedFiles.length > 0) {
                ...
                console.log(
                    `[SessionScope] Filtered to ${scopedFiles.length} session(s) matching scope "${sessionScope}".`,
                );
            } else {
                console.log(
                    `[SessionScope] No sessions match scope "${sessionScope}". Using all ${nonEvolverFiles.length} session(s) (fallback).`,
                );
            }
        }

        const activeFiles =
            nonEvolverFiles.length > 0 ? nonEvolverFiles : files.slice(0, 1);
        ...
        for (let i = 0; i < maxSessions && totalBytes < TARGET_BYTES; i++) {
            ...
            const raw = readRecentLog(
                path.join(AGENT_SESSIONS_DIR, f.name),
                readSize,
            );
...
function getMutationDirective(logContent) {
    ...
    const errorMatches =
        logContent.match(
            /\[ERROR|Error:|Exception:|FAIL|Failed|"isError":true/gi,
        ) || [];
...
const DORMANT_HYPOTHESIS_FILE = path.join(
    getEvolutionDir(),
...
function writeDormantHypothesis(data) {
    try {
        ...
        var obj = Object.assign({}, data, {
            ...
            ttl_ms: DORMANT_TTL_MS,
        });
        var tmp = DORMANT_HYPOTHESIS_FILE + ".tmp";
        fs.writeFileSync(tmp, JSON.stringify(obj, null, 2) + "\n", "utf8");
        fs.renameSync(tmp, DORMANT_HYPOTHESIS_FILE);
        console.log(
            "[DormantHypothesis] Saved partial state before backoff: " +
                (data.backoff_reason || "unknown"),
        );
    } catch (e) {
        console.log(
            "[DormantHypothesis] Write failed (non-fatal): " +
...
function readDormantHypothesis() {
    try {
        ...
        var ttl = Number.isFinite(Number(obj.ttl_ms))
            ? Number(obj.ttl_ms)
            : DORMANT_TTL_MS;
        if (Date.now() - createdAt > ttl) {
            clearDormantHypothesis();
            console.log(
                "[DormantHypothesis] Expired (age: " +
...
function clearDormantHypothesis() {
    try {
        if (fs.existsSync(DORMANT_HYPOTHESIS_FILE))
            fs.unlinkSync(DORMANT_HYPOTHESIS_FILE);
    } catch (e) {}
}
...
const WORKSPACE_ROOT =
    process.env.OPENCLAW_WORKSPACE || path.resolve(REPO_ROOT, "../..");
const ROOT_MEMORY = path.join(WORKSPACE_ROOT, "MEMORY.md");
const DIR_MEMORY = path.join(MEMORY_DIR, "MEMORY.md");
const MEMORY_FILE = fs.existsSync(ROOT_MEMORY)
    ? ROOT_MEMORY
    : fs.existsSync(DIR_MEMORY)
      ? DIR_MEMORY
      : ROOT_MEMORY;
    ...
    try {
        ...
        if (scope) {
            ...
            if (fs.existsSync(scopedMemory)) {
                ...
                console.log(
                    `[SessionScope] Reading scoped MEMORY.md for "${scope}".`,
                );
            } else {
                // First run with scope: global MEMORY.md will be used, but note it.
                console.log(
                    `[SessionScope] No scoped MEMORY.md for "${scope}". Using global MEMORY.md.`,
                );
            }
        }
...
function performMaintenance() {
    ...
    try {
        ...
        if (evolverFiles.length > 0) {
            console.log(
                `[Maintenance] Cleaned ${evolverFiles.length} evolver hand session(s).`,
            );
        }
        ...
        console.log(
            `[Maintenance] Found ${remaining} session logs. Archiving old ones...`,
        );
            ...
            .map((f) => {
                try {
                    return {
                        name: f,
                        time: fs
                            .statSync(path.join(AGENT_SESSIONS_DIR, f))
                            .mtime.getTime(),
                    };
                } catch (e) {
                    return null;
        ...
        if (toArchive.length > 0) {
            console.log(
                `[Maintenance] Archived ${toArchive.length} logs to ${ARCHIVE_DIR}`,
            );
        }
    } catch (e) {
    ...
    try {
        ...
        try {
            if (fs.existsSync(configPath)) {
                ...
                if (cfg.evolver && cfg.evolver.autoUpdate === false)
                    autoUpdate = false;
                if (
                    cfg.evolver &&
                    Number.isFinite(Number(cfg.evolver.autoUpdateIntervalHours))
                ) {
        ...
        try {
            if (fs.existsSync(stateFile)) {
                ...
                if (
                    state.lastCheckedAt &&
                    now - new Date(state.lastCheckedAt).getTime() < intervalMs
                ) {
        ...
        let clawhubBin = null;
        const whichCmd =
            process.platform === "win32" ? "where clawhub" : "which clawhub";
        const candidates = [
            "clawhub",
            path.join(os.homedir(), ".npm-global/bin/clawhub"),
        ...
        for (const slug of slugs) {
            try {
                ...
                if (
                    ...
                    console.log(
                        `[AutoUpdate] ${slug}: ${out.trim().split("\n").pop()}`,
                    );
                    updated = true;
                }
            } catch (e) {
        ...
        if (updated) {
            console.log(
                "[AutoUpdate] Skills updated. Changes will take effect on next wrapper restart.",
            );
        }
    } catch (e) {
        ...
        return fs
            ...
            .filter(
                (f) =>
                    f.endsWith(".jsonl") &&
                    !f.includes(".lock") &&
                    !f.startsWith("evolver_hand_"),
            )
            .filter((f) => {
                try {
                    return (
                        now -
                            fs.statSync(path.join(AGENT_SESSIONS_DIR, f))
                                .mtimeMs <
                        w
                    );
                } catch (_) {
...
async function run() {
    const bridgeEnabled =
        String(process.env.EVOLVE_BRIDGE || "").toLowerCase() !== "false";
    const loopMode =
        ARGS.includes("--loop") ||
        ARGS.includes("--mad-dog") ||
        String(process.env.EVOLVE_LOOP || "").toLowerCase() === "true";
    ...
    if (process.platform !== "win32") {
        try {
            const _psRace = require("child_process")
                .execSync(
            ...
            if (_psRace && _psRace.length > 0) {
                console.log(
                    "[Evolver] Another evolver Hand Agent is already running. Yielding this cycle.",
                );
                return;
            }
        } catch (_) {
    ...
    const QUEUE_MAX = Number.parseInt(
        process.env.EVOLVE_AGENT_QUEUE_MAX || "10",
        10,
    );
    const QUEUE_BACKOFF_MS = Number.parseInt(
        process.env.EVOLVE_AGENT_QUEUE_BACKOFF_MS || "60000",
        10,
    );
    const activeUserSessions = getRecentActiveSessionCount(10 * 60 * 1000);
    if (activeUserSessions > QUEUE_MAX) {
        console.log(
            `[Evolver] Agent has ${activeUserSessions} active user sessions (max ${QUEUE_MAX}). Backing off ${QUEUE_BACKOFF_MS}ms to avoid starving user conversations.`,
        );
    ...
    const LOAD_MAX = parseFloat(
        process.env.EVOLVE_LOAD_MAX || String(getDefaultLoadMax()),
    );
    const sysLoad = getSystemLoad();
    if (sysLoad.load1m > LOAD_MAX) {
        console.log(
            `[Evolver] System load ${sysLoad.load1m.toFixed(2)} exceeds max ${LOAD_MAX.toFixed(1)} (auto-calculated for ${os.cpus().length} cores). Backing off ${QUEUE_BACKOFF_MS}ms.`,
        );
        writeDormantHypothesis({
            backoff_reason: "system_load_exceeded",
            system_load: {
                load1m: sysLoad.load1m,
                load5m: sysLoad.load5m,
                load15m: sysLoad.load15m,
            },
            load_max: LOAD_MAX,
            ...
            if (lastRun && lastRun.run_id) {
                ...
                if (pending) {
                    writeDormantHypothesis({
                        ...
                        signals:
                            lastRun && Array.isArray(lastRun.signals)
                                ? lastRun.signals
                                : [],
                        selected_gene_id:
                            lastRun && lastRun.selected_gene_id
                                ? lastRun.selected_gene_id
                                : null,
                        mutation:
                            lastRun && lastRun.mutation
                                ? lastRun.mutation
                                : null,
                        personality_state:
                            lastRun && lastRun.personality_state
    ...
    try {
        ...
        if (e && e.code === "ENOENT") {
            console.warn(
                "[Evolver] CWD lost (ENOENT). Recovering to REPO_ROOT: " +
                    REPO_ROOT,
            );
            try {
    ...
    try {
        ...
        console.error(
            "[Evolver] FATAL: Not a git repository (" + REPO_ROOT + ").",
        );
        console.error(
            "[Evolver] Evolver requires git for rollback, blast radius calculation, and solidify.",
        );
        console.error(
            '[Evolver] Run "git init && git add -A && git commit -m init" in your project root, then try again.',
        );
        process.exitCode = 1;
        return;
    }
    ...
    if (dormantHypothesis) {
        console.log(
            "[DormantHypothesis] Recovered partial state from previous backoff: " +
                (dormantHypothesis.backoff_reason || "unknown"),
        );
        clearDormantHypothesis();
    }
    ...
    try {
        ...
        console.error(
            `[AssetInit] ensureAssetFiles failed (non-fatal): ${e.message}`,
        );
    }
    ...
    try {
        ...
        const recent = Array.isArray(allEvents)
            ? allEvents.slice(-REPAIR_LOOP_THRESHOLD)
            : [];
        if (recent.length >= REPAIR_LOOP_THRESHOLD) {
            const allRepairFailed = recent.every(
                (e) =>
                    e &&
                    e.intent === "repair" &&
                    e.outcome &&
            ...
            if (allRepairFailed) {
                const geneIds = recent.map(
                    (e) => (e.genes_used && e.genes_used[0]) || "unknown",
                );
                const sameGene = geneIds.every((id) => id === geneIds[0]);
                console.warn(
                    `[CircuitBreaker] Detected ${REPAIR_LOOP_THRESHOLD} consecutive failed repairs${sameGene ? ` (gene: ${geneIds[0]})` : ""}. Forcing innovation intent to break the loop.`,
                );
        ...
        console.error(
            `[CircuitBreaker] Check failed (non-fatal): ${e.message}`,
        );
    }
  ...
  - Changes: Detail exactly what was improved.`;
    ...
    if (process.env.EVOLVE_REPORT_DIRECTIVE) {
        reportingDirective = process.env.EVOLVE_REPORT_DIRECTIVE.replace(
            "__CYCLE_ID__",
            cycleId,
        );
  ...
  - Ensure you pass the status and action details.`;
                ...
                if (isFresh && cacheStats.mtimeMs > dirStats.mtimeMs) {
                    try {
                        const cached = JSON.parse(
                            fs.readFileSync(SKILLS_CACHE_FILE, "utf8"),
                        );
            ...
            if (!useCache) {
                const skills = fs
                    ...
                    .map((dirent) => {
                        ...
                        try {
                            const pkg = require(
                                path.join(skillsDir, name, "package.json"),
                            );
                            if (pkg.description)
                                desc =
                ...
                try {
                    fs.writeFileSync(
                        SKILLS_CACHE_FILE,
                        JSON.stringify({ list: fileList }, null, 2),
                    );
                } catch (e) {}
            }
        }
    ...
    const memorySize = fs.existsSync(MEMORY_FILE)
        ? fs.statSync(MEMORY_FILE).size
        : 0;
    ...
    const recentEvents = (() => {
        try {
            ...
            return Array.isArray(all)
                ? all.filter((e) => e && e.type === "EvolutionEvent").slice(-80)
                : [];
        } catch (e) {
            return [];
        }
    ...
    if (
        dormantHypothesis &&
        Array.isArray(dormantHypothesis.signals) &&
        dormantHypothesis.signals.length > 0
    ) {
        ...
        if (injected > 0) {
            console.log(
                "[DormantHypothesis] Injected " +
                    injected +
                    " signal(s) from previous interrupted cycle.",
            );
        }
    }
    ...
    try {
        ...
        if (proactiveQuestions.length > 0) {
            console.log(
                `[QuestionGenerator] Generated ${proactiveQuestions.length} proactive question(s).`,
            );
        }
    } catch (e) {
        console.log(
            `[QuestionGenerator] Generation failed (non-fatal): ${e.message}`,
        );
    }
    ...
    try {
        ...
        if (
            fetchResult.questions_created &&
            fetchResult.questions_created.length > 0
        ) {
            const created = fetchResult.questions_created.filter(function (q) {
                return !q.error;
            });
            const failed = fetchResult.questions_created.filter(function (q) {
                return q.error;
            });
            if (created.length > 0) {
                console.log(
                    `[QuestionGenerator] Hub accepted ${created.length} question(s) as bounties.`,
                );
            }
            if (failed.length > 0) {
                console.log(
                    `[QuestionGenerator] Hub rejected ${failed.length} question(s): ${failed
                        .map(function (q) {
                            return q.error;
                        })
                        .join(", ")}`,
        ...
        if (
            Array.isArray(fetchResult.relevant_lessons) &&
            fetchResult.relevant_lessons.length > 0
        ) {
            hubLessons = fetchResult.relevant_lessons;
            console.log(
                `[LessonBank] Received ${hubLessons.length} lesson(s) from ecosystem.`,
            );
        }
        ...
        if (hubTasks.length > 0) {
            ...
            if (best) {
                ...
                const claimed =
                    alreadyClaimed ||
                    (await claimTask(best.id || best.task_id));
                if (claimed) {
                    activeTask = best;
                    ...
                    console.log(
                        `[TaskReceiver] ${alreadyClaimed ? "Resuming" : "Claimed"} task: "${best.title || best.id}" (${taskSignals.length} signals injected)`,
                    );
                }
            }
        }
        ...
        console.log(
            `[TaskReceiver] Fetch/claim failed (non-fatal): ${e.message}`,
        );
    }
    ...
    if (!activeTask && process.env.WORKER_ENABLED === "1") {
        try {
            ...
            if (workerTasks.length > 0) {
                ...
                if (best) {
                    const assignment = await claimWorkerTask(
                        best.id || best.task_id,
                    );
                    if (assignment) {
                        activeTask = best;
                        activeTask._worker_assignment_id =
                        ...
                        console.log(
                            `[WorkerPool] Claimed worker task: "${best.title || best.id}" assignment=${activeTask._worker_assignment_id} (${taskSignals.length} signals injected)`,
                        );
                    }
                }
            }
    ...
    const recentErrorMatches =
        recentMasterLog.match(
            /\[ERROR|Error:|Exception:|FAIL|Failed|"isError":true/gi,
        ) || [];
    ...
    if (sessionScope) {
        console.log(
            `[SessionScope] Active scope: "${sessionScope}". Evolution state and memory graph are isolated.`,
        );
    }
    ...
    try {
        ...
        console.error(
            `[MemoryGraph] Refusing to evolve without causal memory. Target: ${memoryGraphPath()}`,
        );
    ...
    try {
        ...
        console.error(
            `[MemoryGraph] Signal snapshot write failed: ${e.message}`,
        );
        console.error(
            `[MemoryGraph] Refusing to evolve without causal memory. Target: ${memoryGraphPath()}`,
        );
    ...
    const capabilityCandidatesPreview = renderCandidatesPreview(
        recentCandidates.slice(-8),
        1600,
    );
    ...
    try {
        ...
        const matchedExternalGenes = genesOnly
            .map((g) => {
                const pats = Array.isArray(g.signals_match)
                    ? g.signals_match
                    : [];
                const hit = pats.reduce(
                    (acc, p) =>
                        matchPatternToSignals(p, signals) ? acc + 1 : acc,
                    0,
                );
                return { gene: g, hit };
            })
        ...
        const matchedExternalCapsules = capsulesOnly
            .map((c) => {
                ...
                const score = triggers.reduce(
                    (acc, t) =>
                        matchPatternToSignals(t, signals) ? acc + 1 : acc,
                    0,
                );
    ...
    try {
        ...
        if (hubHit && hubHit.hit) {
            console.log(
                `[SearchFirst] Hub hit: asset=${hubHit.asset_id}, score=${hubHit.score}, mode=${hubHit.mode}`,
            );
        } else {
            console.log(
                `[SearchFirst] No hub match (reason: ${hubHit && hubHit.reason ? hubHit.reason : "unknown"}). Proceeding with local evolution.`,
            );
        }
    } catch (e) {
        console.log(
            `[SearchFirst] Hub search failed (non-fatal): ${e.message}`,
        );
    ...
    try {
        const cycleState = fs.existsSync(STATE_FILE)
        ...
        if (shouldReflect({ cycleCount, recentEvents })) {
            ...
            recordReflection({
                ...
                preferred_gene:
                    memoryAdvice && memoryAdvice.preferredGeneId
                        ? memoryAdvice.preferredGeneId
                        : null,
                banned_genes:
                    memoryAdvice && Array.isArray(memoryAdvice.bannedGeneIds)
                        ? memoryAdvice.bannedGeneIds
                        : [],
                context_preview: reflectionCtx.slice(0, 1000),
            });
            console.log(
                `[Reflection] Strategic reflection recorded at cycle ${cycleCount}.`,
            );
        }
    } catch (e) {
        console.log(
    ...
    const personalityState =
        personalitySelection && personalitySelection.personality_state
    ...
    const tailOutcomes = tail
        .map((e) =>
            e && e.outcome && e.outcome.status
                ? String(e.outcome.status)
                : null,
        )
        .filter(Boolean);
    const stableSuccess =
        tailOutcomes.length >= 6 && tailOutcomes.every((s) => s === "success");
    const tailAvgScore =
        tail.length > 0
            ? tail.reduce(
                  (acc, e) =>
                      ...
                      (e &&
                      e.outcome &&
                      Number.isFinite(Number(e.outcome.score))
                          ? Number(e.outcome.score)
                          : 0),
                  0,
              ) / tail.length
            : 0;
    ...
    const forceInnovation =
        String(
            process.env.FORCE_INNOVATION ||
                process.env.EVOLVE_FORCE_INNOVATION ||
                "",
        ).toLowerCase() === "true";
    const mutationInnovateMode =
        !!IS_RANDOM_DRIFT || !!innovationPressure || !!forceInnovation;
    const mutationSignals = innovationPressure
        ? [...(Array.isArray(signals) ? signals : []), "stable_success_plateau"]
        : signals;
    const mutationSignalsEffective = forceInnovation
        ? [
              ...(Array.isArray(mutationSignals) ? mutationSignals : []),
              "force_innovation",
          ]
        : mutationSignals;
    ...
    try {
        ...
        const maxFiles =
            selectedGene &&
            selectedGene.constraints &&
            Number.isFinite(Number(selectedGene.constraints.max_files))
        ...
        prevState.last_run = {
            ...
            selected_gene_id:
                selectedGene && selectedGene.id ? selectedGene.id : null,
            ...
            personality_key:
                personalitySelection && personalitySelection.personality_key
                    ? personalitySelection.personality_key
                    : null,
            personality_known: !!(
                personalitySelection && personalitySelection.personality_known
            ),
            personality_mutations:
                personalitySelection &&
                Array.isArray(personalitySelection.personality_mutations)
            ...
            source_type:
                hubHit && hubHit.hit
                    ...
                    : "generated",
            reused_asset_id:
                hubHit && hubHit.hit ? hubHit.asset_id || null : null,
            reused_source_node:
                hubHit && hubHit.hit ? hubHit.source_node_id || null : null,
            reused_chain_id:
                hubHit && hubHit.hit ? hubHit.chain_id || null : null,
            ...
            active_task_id: activeTask
                ? activeTask.id || activeTask.task_id || null
                : null,
            active_task_title: activeTask ? activeTask.title || null : null,
            worker_assignment_id: activeTask
                ? activeTask._worker_assignment_id || null
                : null,
            applied_lessons: hubLessons
                .map(function (l) {
                    return l.lesson_id;
                })
                .filter(Boolean),
        ...
        if (hubHit && hubHit.hit) {
            const assetAction =
                hubHit.mode === "direct" ? "asset_reuse" : "asset_reference";
            logAssetCall({
                run_id: runId,
                action: assetAction,
                asset_id: hubHit.asset_id || null,
                asset_type:
                    hubHit.match && hubHit.match.type
                        ? hubHit.match.type
                        : null,
    ...
    const recentHistorySummary = (() => {
        if (!recentEvents || recentEvents.length === 0)
            return "(no prior evolution events)";
        const last8 = recentEvents.slice(-8);
        const lines = last8.map((evt, idx) => {
            const sigs = Array.isArray(evt.signals)
                ? evt.signals.slice(0, 3).join(", ")
                : "?";
            const gene =
                Array.isArray(evt.genes_used) && evt.genes_used.length
                    ? evt.genes_used[0]
                    : "none";
            const outcome =
                evt.outcome && evt.outcome.status ? evt.outcome.status : "?";
...
${(() => {
    ...
    for (let i = evts.length - 1; i >= 0; i--) {
        if (evts[i] && evts[i].outcome && evts[i].outcome.status === "failed")
            cfc++;
        else break;
    }
    if (cfc >= 3) {
...
`.trim();
    ...
    const hubMatchedBlock =
        hubHit && hubHit.hit && hubHit.mode === "reference"
    ...
    const emitThought =
        String(process.env.EVOLVE_EMIT_THOUGHT_PROCESS || "").toLowerCase() ===
        "true";
    if (emitThought) {
    ...
    const printPrompt =
        String(process.env.EVOLVE_PRINT_PROMPT || "").toLowerCase() === "true";
