...
export default function Edit() {
    ...
    const [form] = useForm({
        ...
        foto_profile_preview: user.foto_profile
            ? `/storage/images/profile/${user.foto_profile}`
            : '',
        linkidln: user.userDetail?.linkidln ?? '',
        instagram: user.userDetail?.instagram ?? '',
        website: user.userDetail?.website ?? '',
        facebook: user.userDetail?.facebook ?? '',
        twitter: user.userDetail?.twitter ?? '',
        youtube: user.userDetail?.youtube ?? '',
    ...
    const handleImageChange = (e) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                ...
                form.set('foto_profile_preview', result);
            };
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('backend-pengguna-ppdb.update', user.id), form);
        form.reset();
    };
    ...
    return (
        <>
            <div className="mb-6">
                <h1 className="text-2xl font-bold">Edit Staf PPDB</h1>
                <Link
                    href={route('backend-pengguna-ppdb.index')}
                    className="btn btn-outline mb-2"
                >
                    Kembali ke Daftar Staf PPDB
                </Link>
                {flash?.success && (
            ...
            <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 space-y-6">
                ...
                <div className="flex justify-end space-x-2">
                    <button
                        ...
                        onClick={() => {
                            form.reset();
                            // reset to original? we'll just reload? but we can ignore.
                        }}
