...
export default function Edit() {
    ...
    const [form] = useForm({
        ...
        nip: bendahara.userDetail?.nip ?? '',
        foto_profile: null,
        foto_profile_preview: bendahara.foto_profile
            ? `/storage/images/profile/${bendahara.foto_profile}`
            : '',
        linkidln: bendahara.userDetail?.linkidln ?? '',
        instagram: bendahara.userDetail?.instagram ?? '',
        website: bendahara.userDetail?.website ?? '',
        facebook: bendahara.userDetail?.facebook ?? '',
        twitter: bendahara.userDetail?.twitter ?? '',
        youtube: bendahara.userDetail?.youtube ?? '',
        status: bendahara.status ?? 'Aktif',
    });
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('backend-pengguna-bendahara.update', bendahara.id), form);
    };

    return (
        <>
            <div className="mb-6">
                <h1 className="text-2xl font-bold">Edit Bendahara</h1>
                <Link
                    href={route('backend-pengguna-bendahara.index')}
            ...
            <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 space-y-6">
                ...
                <div>
                    ...
                    {!!form.errors.status && <span className="text-error text-sm">{form.errors.status}</span>}
                </div>
