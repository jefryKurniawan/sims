...
import { Loader2, MapPin, CheckCircle, AlertCircle, XCircle, Home, LogOut, Info } from 'lucide-react';
...
export default function Checkin({ auth, settings }: Props) {
    ...
    const jamMasuk = settings.absensi_jam_masuk || '07:00';
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        form.post('/api/absensi/checkin', {
            ...
            onError: (err) => {
                setError(err.message || 'Absen gagal');
            },
        });
    };
    ...
    const isLate = currentTime > jamMasuk;

    if (countdown > 0) {
        return (
            <>
                <Head title="Absen Masuk - Berhasil" />
                <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-emerald-100 p-4">
                    <Card className="w-full max-w-md text-center">
                        <CardContent className="py-12 px-6">
                            ...
                            <h1 className="text-2xl font-bold text-green-800">Absen Masuk Berhasil!</h1>
    ...
    return (
        <>
            <Head title="Absen Masuk - GPS Check-in" />
            <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white">
                <div className="max-w-md mx-auto p-4 pt-8 pb-16">
                    <div className="text-center mb-8">
                        <div className="w-16 h-16 bg-blue-600 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg">
                            <GPS className="w-8 h-8 text-white" />
                        </div>
                        <h1 className="text-2xl font-bold text-gray-900">Absen Masuk</h1>
                        <p className="text-gray-500 mt-1">{currentDate}</p>
                        <p className="text-3xl font-mono font-bold text-blue-600 mt-2">{currentTime}</p>
                        {isLate && (
                            <Badge className="mt-2 inline-flex bg-yellow-100 text-yellow-800 text-sm" variant="secondary">
                                <Clock className="w-3 h-3 mr-1" />
                                Terlambat (batas {jamMasuk})
                            </Badge>
                        )}
                    </div>
                        ...
                        <CardContent className="space-y-4">
                                    ...
                                    <div className={`flex items-center gap-3 p-3 rounded-lg ${
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-500">
                                                Status absen: {inRadius === true ? (isLate ? 'TERLAMBAT' : 'HADIR') : 'ALPA (di luar radius)'}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm">
                                <p className="font-medium text-blue-800 flex items-center gap-1">
                                    ...
                                    Radius sekolah: {radiusKm}km | Batas jam masuk: {jamMasuk}
                                </p>
                                <p className="text-blue-700 mt-1">Pastikan GPS HP aktif & izin lokasi diberikan ke browser</p>
                            </div>
                        </CardContent>
                    </Card>
                        ...
                        >
                            {form.processing ? (
                                ...
                                <>
                                    ...
                                    {isLate ? 'Absen Masuk (TERLAMBAT)' : 'Absen Masuk (HADIR)'}
                                </>
                            ) : (
                                <>
                                    <AlertCircle className="w-5 h-5 mr-2" />
                                    Absen Masuk (ALPA - di luar radius)
                                </>
                            )}
                        </Button>
                    </form>
                    ...
                    <div className="mt-6 space-y-2">
                        <Link href="/absensi/checkout" className="block w-full">
