...
export default function Index() {
    ...
    const handleProsesScoring = () => {
        ...
        if (
            !window.confirm(
                "Proses scoring & ranking akan memproses semua pendaftar pada periode ini. Lanjutkan?",
            )
        )
            return;
    ...
    const getJalurBadge = (jalur: string) => {
        const classes: Record<string, string> = {
            ...
            afirmasi:
                "px-2 py-0.5 text-xs font-medium rounded-full bg-purple-100 text-purple-700",
            prestasi:
            ...
            <div className="p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-6">
                    {[
                        ...
                        {
                            label: "Submitted",
                            value: statistik.submitted,
                            color: "bg-blue-500",
                        },
                        {
                            label: "Verifikasi",
                            value: statistik.verifikasi_berkas,
                            color: "bg-yellow-500",
                        },
                        {
                            label: "Lulus",
                            value: statistik.lulus_seleksi,
                            color: "bg-green-500",
                        },
                        {
                            label: "Diterima",
                            value: statistik.diterima,
                        ...
                        >
                            ...
                            <p
                                className={`text-2xl font-bold text-white px-2 py-1 rounded ${card.color} inline-block mt-1`}
                            >
                                {card.value}
                            </p>
                        </div>
                    ...
                    <div className="flex flex-wrap items-end gap-4">
                        <div>
                            ...
                            <select
                                ...
                                className="w-full px-3 py-2 border border-primary/20 rounded-md focus:outline-none focus:ring-2 focus:ring-ring dark:bg-gray-700 dark:border-gray-600 min-w-[200px]"
                            >
                ...
                >
                    <div className="flex items-end gap-4">
                        ...
                        <a
                            href="/dashboard/spmb/ranking"
                            className="px-4 py-2 border border-primary/20 text-gray-700 rounded-md hover:bg-gray-50 text-sm font-medium inline-block"
                        >
                            Reset
                        </a>
                    </div>
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {rankings.length > 0 ? (
                                rankings.map((r: any) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3 font-mono text-sm">
                                            {r.applicant?.nomor_registrasi}
                                        </td>
                                        <td className="px-4 py-3">
                                            <div className="font-semibold">
                                                {r.applicant?.nama_lengkap}
                                            </div>
                                        ...
                                        <td className="px-4 py-3">
                                            {getJalurBadge(r.jalur)}
                                        </td>
                                        <td className="px-4 py-3">
                                            {Number(r.skor_rapor).toFixed(2)}
                                        </td>
                                        <td className="px-4 py-3">
                                            {Number(r.skor_tka).toFixed(2)}
                                        </td>
                                        <td className="px-4 py-3">
                                            {Number(r.skor_prestasi).toFixed(2)}
                                        </td>
                                        <td className="px-4 py-3">
                                            {Number(r.bonus_afirmasi).toFixed(
                                                2,
                                            )}
                                        </td>
                                        <td className="px-4 py-3 font-bold text-blue-600">
                                            {Number(r.skor_total).toFixed(2)}
                                        </td>
                                        <td className="px-4 py-3">
                                            {getStatusBadge(r.lulus_seleksi)}
                                        </td>
                                    </tr>
                                ))
                            ) : (
                                <tr>
                                    ...
                                    >
                                        Belum ada data ranking. Jalankan proses
                                        scoring & ranking terlebih dahulu.
                                    </td>
                                </tr>
                            )}
                        </tbody>
