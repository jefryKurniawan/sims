import { Head, usePage, Link } from '@inertiajs/inertia-react';
import { Inertia } from '@inertiajs/inertia';
import { useState } from 'react';
import AdminTable from '@/Components/AdminTable';
import type { Column } from '@/Components/AdminTable';
import ConfirmModal from '@/Components/ConfirmModal';

export default function Index() {
  const { bendahara, flash } = usePage().props as {
    bendahara: any;
    flash: { success?: string; error?: string };
  };
  const [deleteTarget, setDeleteTarget] = useState<any>(null);

  const handleDelete = () => {
    if (!deleteTarget) return;
    Inertia.delete(route('users.bendahara.destroy', deleteTarget.id));
    setDeleteTarget(null);
  };

  const columns: Column[] = [
    { key: 'name', label: 'Nama' },
    { key: 'email', label: 'Email' },
    { key: 'nip', label: 'NIP', render: (_v: any, row: any) => row.userDetail?.nip || '-' },
    {
      key: 'status',
      label: 'Status',
      render: (v: string) => (
        <span className={`inline-flex px-2 py-0.5 text-xs font-medium rounded-full ${v === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
          {v === 'active' ? 'Aktif' : 'Nonaktif'}
        </span>
      ),
    },
  ];

  return (
    <>
      <Head title="Pengguna Bendahara" />
      <div className="p-4 lg:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
          <div>
            <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 font-heading">Pengguna Bendahara</h1>
            <p className="text-sm text-gray-500 mt-0.5">Kelola akun bendahara sekolah</p>
          </div>
          <Link
            href={route('users.bendahara.create')}
            className="inline-flex items-center gap-2 px-4 py-2.5 bg-school-red text-white rounded-lg hover:bg-red-700 transition text-sm font-semibold shadow-sm"
          >
            Bendahara Baru
          </Link>
        </div>

        {flash?.success && <div className="mb-4 p-4 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-lg text-sm font-medium">{flash.success}</div>}
        {flash?.error && <div className="mb-4 p-4 bg-red-50 text-red-700 border border-red-200 rounded-lg text-sm font-medium">{flash.error}</div>}

        <AdminTable
          columns={columns}
          rows={bendahara?.data || []}
          pagination={{
            current_page: bendahara?.current_page,
            last_page: bendahara?.last_page,
            per_page: bendahara?.per_page,
            from: bendahara?.from,
            to: bendahara?.to,
            total: bendahara?.total,
            links: bendahara?.links,
          }}
          actions={(row) => [
            { icon: 'edit', onClick: () => Inertia.visit(route('users.bendahara.edit', row.id)), label: 'Edit' },
            { icon: 'delete', onClick: () => setDeleteTarget(row), label: 'Hapus' },
          ]}
        />

        <ConfirmModal
          open={!!deleteTarget}
          title="Hapus Bendahara"
          message={`Yakin ingin menghapus "${deleteTarget?.name}"?`}
          onConfirm={handleDelete}
          onCancel={() => setDeleteTarget(null)}
        />
      </div>
    </>
  );
}
