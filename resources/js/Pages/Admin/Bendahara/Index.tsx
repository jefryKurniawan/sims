...
export default function Index({ bendahara }: Props) {
    const { flash } = usePage().props;

    const handleDelete = (id: number) => {
        if (confirm('Apakah anda yakin ingin menghapus pengguna Bendahara ini?')) {
            Inertia.delete(route('users.bendahara.destroy', id));
        }
    };

    return (
        <AdminLayout title="Pengguna Bendahara">
            <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                    <h1 className="text-2xl font-bold text-gray-800">Pengguna Bendahara</h1>
                    <Link
                        href={route('users.bendahara.create')}
                        className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700"
                    >
                        Tambah Pengguna Bendahara
                    </Link>
                </div>
                                ...
                                <tr key={item.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex gap-2">
                                            <Link
                                                href={route('users.bendahara.edit', item.id)}
                            ...
                            {bendahara.length === 0 && (
                                <tr>
                                    <td colSpan={6} className="px-4 py-8 text-center text-sm text-gray-500">
                                        Tidak ada data pengguna Bendahara
                                    </td>
                                </tr>
                            )}
                        </tbody>
