    ...
    const columns: Column[] = [
        {
            key: "user",
            label: "Nama",
            render: (_v: any, row: any) => row.user?.name || "Tidak Diketahui",
        },
    ...
    return (
        <>
            ...
            <div className="p-4 lg:p-6">
                ...
                {flash?.error && (
                    <div className="mb-4 p-4 bg-destructive/10 text-red-50 border border-red-200 rounded-lg text-sm font-medium">
                        {flash.error}
                    </div>
                )}
                    ...
                    actions={(row) => [
                        {
                            icon: "eye",
                            onClick: () =>
                                Inertia.visit(route("alumni.show", row.id)),
                            label: "Detail",
                        },
                        {
                            icon: "edit",
                            onClick: () =>
                                Inertia.visit(route("alumni.edit", row.id)),
                            label: "Edit",
                        },
                        {
                            icon: "delete",
                ...
                <ImportModal
                    ...
                    title="Import Alumni"
                    templateRouteXlsx={
                        route("alumni.template") + "?format=xlsx"
                    }
