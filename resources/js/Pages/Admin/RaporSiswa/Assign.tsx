...
export default function Assign({ kelas, siswa }: Props) {
    ...
    const toggleSiswa = (id: number) => {
        ...
        setData('siswa_ids', selectedSiswa.includes(id)
    ...
    return (
        <AdminLayout title="Assign Siswa ke Kelas Rapor">
            <div className="mb-6">
                <h1 className="text-2xl font-bold text-gray-800">Assign Siswa ke Kelas Rapor</h1>
            </div>
            ...
            <form onSubmit={handleSubmit}>
                <div className="bg-white rounded-lg border p-6 mb-6">
                    <h2 className="text-lg font-semibold mb-4">Konfigurasi Rapor</h2>
                ...
                <div className="bg-white rounded-lg border overflow-hidden">
                    <div className="px-4 py-3 border-b bg-gray-50">
                        ...
                        <p className="text-sm text-gray-500 mt-1">
                            {siswa.length} siswa tersedia (belum terdaftar di semester & tahun ajaran ini)
                        </p>
                    </div>

                    {siswa.length === 0 ? (
                        <div className="p-6 text-center text-sm text-gray-500">
                            Semua siswa sudah terdaftar di semester dan tahun ajaran ini
                        </div>
                    ) : (
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="bg-gray-50">
                                        ...
                                        <th className="px-4 py-2 text-left text-xs font-semibold text-gray-600 uppercase">NISN</th>
                                        <th className="px-4 py-2 text-left text-xs font-semibold text-gray-600 uppercase">Nama Lengkap</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y">
                                    {siswa.map((s) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-2 text-sm text-gray-700">{s.nisn}</td>
                                            <td className="px-4 py-2 text-sm font-medium text-gray-900">{s.nama_lengkap}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                    ...
                    <div className="px-4 py-3 border-t bg-gray-50 flex justify-between items-center">
                        ...
                        <div className="flex gap-2">
                            ...
                            >
                                {processing ? 'Menyimpan...' : `Assign ${selectedSiswa.length} Siswa`}
                            </button>
                        </div>
                    </div>
                </div>
