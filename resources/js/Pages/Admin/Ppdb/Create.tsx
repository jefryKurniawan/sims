...
export default function Create({ users, role }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (isEdit) {
            ...
            Inertia.post(route('users.ppdb.update', users.id), formData);
        } else {
            Inertia.post(route('users.ppdb.store'), formData);
        }
    };

    return (
        <AdminLayout title={isEdit ? 'Edit Pengguna PPDB' : 'Tambah Pengguna PPDB'}>
            <div className="p-6">
                <div className="mb-6">
                    <h1 className="text-2xl font-bold text-gray-800">{isEdit ? 'Edit Pengguna PPDB' : 'Tambah Pengguna PPDB'}</h1>
                </div>

                <div className="bg-white rounded-lg border">
                    <div className="px-6 py-4 border-b">
                        <h4 className="text-lg font-semibold text-gray-800">{isEdit ? 'Edit Pengguna PPDB' : 'Tambah Pengguna PPDB'}</h4>
                    </div>
                    <div className="p-6">
