...
export default function Index({ berita, kategori, filters }: Props) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        Inertia.get(route('berita-admin.index'), { search }, { preserveState: true, replace: true });
    };
            ...
            <div className="p-6">
                ...
                {kategori.length === 0 ? (
                    <div className="p-6 text-center text-gray-500 bg-white rounded-lg border">
                        <h3 className="text-lg font-semibold">Kategori Masih Kosong!</h3>
                        <p className="mt-2">Silakan tambah kategori berita terlebih dahulu.</p>
                    </div>
                ) : (
                    <>
                                        ...
                                        <tr key={item.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-4 py-3">
                                                <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                                                    item.is_active === '0' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                                                }`}>
                                                    {item.is_active === '0' ? 'Publish' : 'Draft'}
                                                </span>
                                            </td>
                                            <td className="px-4 py-3">
                                                <div className="flex gap-2">
                                                    <Link
                                                        href={route('berita-admin.edit', item.id)}
