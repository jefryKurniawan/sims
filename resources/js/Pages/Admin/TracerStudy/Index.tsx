...
export default function Index() {
    const { tracerStudies, filters = { status: '', search: '' }, stats } = usePage().props as any;
    ...
    const getStatusBadge = (status: string) => {
        const badges: Record<string, { bg: string; text: string; label: string }> = {
            bekerja: { bg: 'bg-emerald-100', text: 'text-emerald-700', label: 'Bekerja' },
            kuliah: { bg: 'bg-blue-100', text: 'text-blue-700', label: 'Kuliah' },
            wirausaha: { bg: 'bg-purple-100', text: 'text-purple-700', label: 'Wirausaha' },
            tidak_bekerja: { bg: 'bg-gray-100', text: 'text-gray-700', label: 'Tidak Bekerja' },
        };
        const b = badges[status] || badges.tidak_bekerja;
        return (
            <span className={`inline-flex px-2 py-0.5 text-xs font-medium rounded-full ${b.bg} ${b.text}`}>
                {b.label}
            </span>
        );
    };
    ...
    const columns: Column[] = [
        {
            ...
            render: (_v: any, row: any) => (
                <div>
                    <div className="text-sm font-semibold text-gray-900">{row.nama_lengkap || row.alumni?.user?.name || '-'}</div>
                    {row.no_telp && <div className="text-xs text-gray-500">{row.no_telp}</div>}
                </div>
            ),
        },
        { key: 'tahun_lulus', label: 'Tahun Lulus', render: (v: any) => v || '-' },
        { key: 'status', label: 'Status', render: (v: string) => getStatusBadge(v) },
        {
            key: 'instansi',
            label: 'Instansi / Pendidikan',
            render: (_v: any, row: any) => (
                <div className="text-sm text-gray-900">{row.jenjang_pendidikan || row.nama_instansi || '-'}</div>
            ),
        },
        { key: 'bidang_studi', label: 'Bidang Studi', render: (v: any) => v || '-' },
    ];
        ...
        <>
            ...
            <div className="p-4 lg:p-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
                    <div>
                        <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 font-heading">Tracer Study Alumni</h1>
                        <p className="text-sm text-gray-500 mt-0.5">Data perkembangan alumni setelah lulus</p>
                    </div>
                    <Link
                ...
                {stats && (
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
                        ...
                        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-5">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-sm text-gray-500 mb-1">Bekerja</p>
                        ...
                        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-5">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-sm text-gray-500 mb-1">Kuliah</p>
                        ...
                        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-5">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-sm text-gray-500 mb-1">Wirausaha</p>
                        ...
                        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-5">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-sm text-gray-500 mb-1">Belum Bekerja</p>
                ...
                <AdminTable
                    ...
                    actions={(row) => [
                        { icon: 'eye', onClick: () => router.visit(route('admin.tracer-study.show', row.id)), label: 'Detail' },
                        { icon: 'edit', onClick: () => router.visit(route('admin.tracer-study.edit', row.id)), label: 'Edit' },
