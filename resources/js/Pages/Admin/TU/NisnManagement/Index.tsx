...
import { Search, RefreshCw, ArrowRightLeft, RotateCcw } from "lucide-react";
...
export default function Index() {
    const {
        siswa,
        filters = { search: "", nisn_status: "", jurusan_id: "" },
        stats,
        jurusanList,
    ...
    const handleFilter = (key: string, value: string) => {
        ...
        delete params.page;
        router.get(route("tu.nisn-management.index"), params);
    };

    const handleSelectAll = (checked: boolean) => {
        if (checked) {
            setSelectedRows(siswa.data.map((s: any) => s.id));
        } else {
            setSelectedRows([]);
        }
    };
    ...
    const handleRegenerateConfirm = () => {
        ...
        router.post(
            route("tu.nisn-management.regenerate", showRegenerateModal.id),
            {
    ...
    const handleBulkRegenerateConfirm = () => {
        ...
        router.post(
            route("tu.nisn-management.bulk-regenerate"),
            {
                siswa_ids: selectedRows,
                reason: bulkRegenerateReason,
            },
            {
                onSuccess: () => {
    ...
    const handleSyncDapodik = () => {
        ...
        setSyncStatus(null);
        router.post(
            route("tu.nisn-management.sync-dapodik"),
            {},
            {
                onSuccess: (page) => {
                    if (page.props.syncResult) {
                        const { synced, total_received, unmapped } =
                            page.props.syncResult;
                        setSyncStatus({
                            synced,
    ...
    const handleVerify = async (s: any) => {
        const res = await router.post(
            route("tu.nisn-management.verify", s.id),
            {},
        );
        return res;
    };
    ...
    const getStatusBadge = (s: any) => {
        ...
        if (s.nisn && (s.nisn.length !== 10 || !/^[0-9]{10}$/.test(s.nisn)))
    ...
    const columns: Column[] = [
            ...
            render: (v: string, row: any) => (
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="p-1 text-gray-400 hover:text-primary hover:bg-primary/10 rounded transition"
        ...
        },
        {
            key: "jurusan",
            label: "Jurusan",
            render: (v: string, row: any) => row.jurusan?.nama_jurusan || "-",
        },
        {
            key: "kelas",
            label: "Kelas",
            render: (v: string, row: any) =>
                row.kelasAktif?.kelas?.nama_kelas || "-",
        },
        {
            key: "status",
            label: "Status NISN",
            render: (v: string, row: any) => getStatusBadge(row),
        },
        {
            key: "actions",
            label: "Aksi",
            render: (v: string, row: any) => (
                <div className="flex items-center gap-1">
                    <Link
                        href={route("tu.nisn-management.show", row.id)}
        ...
        <>
            ...
            <div className="p-4 lg:p-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-0.5">
                            Verifikasi, regenerate, dan kelola NISN siswa
                        </p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                        <button
                            onClick={handleBulkRegenerateOpen}
                        ...
                        >
                            <RefreshCw className="w-4 h-4" />
                            Regenerate Terpilih ({selectedRows.length})
                        </button>
                        <button
                            onClick={handleSyncDapodik}
                            className="inline-flex items-center gap-2 px-4 py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 transition"
                        >
                            <ArrowRightLeft className="w-4 h-4" />
                            Sync Dapodik
                        </button>
                ...
                <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-4 mb-4">
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                        <div>
                            ...
                            <input
                                type="text"
                                placeholder="Nama, NISN, NIS, No HP..."
                        ...
                        <div>
                            ...
                            <select
                                value={filters.nisn_status}
                                onChange={(e) =>
                                    handleFilter("nisn_status", e.target.value)
                                }
                                className="w-full px-3 py-2 border border-primary/20 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring/20 focus:border-ring"
                            >
                        ...
                        <div>
                            ...
                            <select
                                ...
                                onChange={(e) =>
                                    handleFilter("jurusan_id", e.target.value)
                                }
                                className="w-full px-3 py-2 border border-primary/20 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring/20 focus:border-ring"
                            >
                    ...
                    actions={(row) => [
                        {
                            ...
                            onClick: () =>
                                router.visit(
                                    route("tu.nisn-management.show", row.id),
                                ),
                            label: "Detail",
                        },
                        {
                ...
                <ConfirmModal
                    ...
                    inputType="textarea"
                    inputPlaceholder="Alasan regenerate..."
                    inputValue={regenerateReason}
                    onInputChange={setRegenerateReason}
                    inputRequired
                />

                <ConfirmModal
                    open={showBulkRegenerateModal}
                    title="Bulk Regenerate NISN"
                    message={`Regenerate NISN untuk ${selectedRows.length} siswa terpilih?`}
                    onConfirm={handleBulkRegenerateConfirm}
                    onCancel={() => {
                        setShowBulkRegenerateModal(false);
                        setBulkRegenerateReason("");
                    }}
                    confirmButtonText="Regenerate Semua"
                    inputType="textarea"
                    inputPlaceholder="Alasan regenerate massal..."
                    inputValue={bulkRegenerateReason}
                    onInputChange={setBulkRegenerateReason}
                    inputRequired
                />
                            ...
                            {syncStatus ? (
                                <div className="space-y-4">
                                    <div className="text-center">
                                        <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-green-100 mb-3">
                                            <RotateCcw className="w-8 h-8 text-green-600 animate-spin" />
                                        </div>
                                    ...
                                    {syncStatus.unmapped.length > 0 && (
                                        <div className="max-h-48 overflow-y-auto bg-gray-50 rounded-lg p-3">
                                            <p className="text-sm font-medium text-gray-700 mb-2">
                                                Siswa tidak ditemukan di sistem
                                                lokal:
                                            </p>
                                ...
                                <div className="text-center py-8">
                                    ...
                                    <p className="text-gray-600">
                                        Mengambil data siswa dari Dapodik...
                                    </p>
                                </div>
                            )}
                        </div>
