...
export default function Edit({ suratKeluar }: { suratKeluar: any }) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        form.post(route("tu.surat-keluar.update", suratKeluar.id), {
            _method: "PUT",
            forceFormData: true,
        });
    };
            ...
            <div className="p-4 lg:p-6">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-0.5">{suratKeluar.no_surat}</p>
                    </div>
                    <Link
                ...
                <div className="bg-white border border-gray-200 shadow-sm p-6 max-w-2xl">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium mb-1.5">Tanggal Kirim</label>
                            <input
                                type="date"
                            ...
                            />
                            {form.errors.tanggal_kirim && <span className="text-destructive text-xs">{form.errors.tanggal_kirim}</span>}
                        </div>

                        <div>
                            <label className="block text-sm font-medium mb-1.5">Tujuan</label>
                            <input
                                type="text"
                                ...
                                className="w-full px-3 py-2 border border-primary/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-ring/20 focus:border-ring transition text-sm"
                            />
                            {form.errors.tujuan && <span className="text-destructive text-xs">{form.errors.tujuan}</span>}
                        </div>
                        ...
                        <div>
                            <label className="block text-sm font-medium mb-1.5">Penandatangan</label>
                            <input
                                type="text"
                                ...
                                className="w-full px-3 py-2 border border-primary/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-ring/20 focus:border-ring transition text-sm"
                            />
                            {form.errors.penandatangan && <span className="text-destructive text-xs">{form.errors.penandatangan}</span>}
                        </div>
