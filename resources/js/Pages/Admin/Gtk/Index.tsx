...
export default function Index() {
    const { guru, flash } = usePage().props as {
        guru: any;
    ...
    const columns: Column[] = [
        ...
        { key: "nuptk", label: "NUPTK", render: (v: string) => v || "-" },
        {
            ...
            render: (v: string) => (
                <span
                    className={`inline-flex px-2 py-0.5 text-xs font-medium rounded-full ${v === "Guru" ? "bg-indigo-100 text-indigo-700" : "bg-cyan-100 text-cyan-700"}`}
                >
                    {v}
                </span>
            ),
        ...
        { key: "no_telp", label: "No. Telp", render: (v: string) => v || "-" },
        {
            ...
            render: (v: string) => {
                ...
                return (
                    <span
                        className={`inline-flex px-2 py-0.5 text-xs font-medium rounded-full ${map[v] || map.Honorer}`}
                    >
                        {label[v] || v}
                    </span>
                );
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-0.5">
                            Kelola data Guru & Tenaga Kependidikan
                        </p>
                    </div>
                    ...
                    actions={(row) => [
                        {
                            icon: "eye",
                            onClick: () =>
                                Inertia.visit(route("gtk.show", row.id)),
                            label: "Detail",
                        },
                        {
                            icon: "edit",
                            onClick: () =>
                                Inertia.visit(route("gtk.edit", row.id)),
                            label: "Edit",
                        },
                        {
                            icon: "delete",
