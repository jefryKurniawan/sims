...
export default function Tingkat({ tingkat, variants, kelasList }: Props) {
    const { siswa, flash } = usePage().props as unknown as { siswa: any; flash?: any; [k: string]: any };
    ...
    const handleFilterChange = (variant: string) => {
        ...
        params.set('page', '1');
        Inertia.visit(route('users.murid.tingkat', tingkat) + '?' + params.toString());
    };
        ...
        // ponytail: kolom checkbox pemilihan siswa paling kiri
        {
            ...
            render: (_v: any, row: SiswaRow) => (
                <input
                    ...
                    className="w-4 h-4 rounded border-gray-300 text-school-red focus:ring-red-200 cursor-pointer"
                />
            ),
        },
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-0.5">
                                Filter per variant, import template, atau pindahkan siswa antar kelas
                          </p>
                      </div>
                  </div>
                ...
                <div className="bg-white border border-gray-200 shadow-sm rounded-2xl overflow-hidden">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-4 border-b border-gray-100">
                        <div className="flex items-center gap-3 flex-wrap">
                            ...
                            {variants.map((v) => {
                                const active = new URL(window.location.href).searchParams.get('variant') === v;
                                return (
                                    <button
                                        key={v}
                                        onClick={() => handleFilterChange(v)}
                                        className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${
                                            active ? 'bg-school-red text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                                        }`}
                                    >
                                        {tingkat}
                                        {v}
                            ...
                            { icon: 'edit', onClick: () => Inertia.visit(route('users.murid.edit', row.id)), label: 'Edit' },
                            {
                                ...
                                onClick: () => {
                                    if (confirm(`Hapus ${row.nama_lengkap}?`)) {
                                        Inertia.delete(route('users.murid.destroy', row.id));
                                    }
                                },
                                label: 'Hapus',
                            },
                ...
                {showMigrate && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
                        <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
                            <h3 className="text-lg font-bold text-gray-900 mb-1">Pindahkan Siswa</h3>
                            <p className="text-sm text-gray-500 mb-4">
                                {selectedIds.length} siswa akan dipindahkan ke kelas tujuan.
                          </p>
                            <label className="block text-sm font-semibold text-gray-700 mb-2">Kelas Tujuan</label>
                            <select
                                value={targetKelasId}
                                onChange={(e) => setTargetKelasId(e.target.value)}
                                className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-200 focus:border-amber-400"
                            >
                                <option value="">Pilih kelas tujuan</option>
                                {kelasList.map((k: any) => (
                                    <option key={k.id} value={k.id}>
                                        {k.nama_kelas} {k.jurusan?.nama ? `(${k.jurusan.nama})` : ''}
                                  </option>
                                ))}
                          </select>
