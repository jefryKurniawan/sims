...
export default function Landing() {
    const { angkatan, flash } = usePage().props as unknown as {
        angkatan: Angkatan[];
        flash?: any;
    };

    // ponytail: kasih visual beda untuk tingkatan yang belum punya kelas
    const colorByTingkat: Record<
        string,
        { bg: string; text: string; chip: string; icon: string }
    > = {
        "10": {
            ...
            <div className="p-4 lg:p-6">
                <div className="mb-6">
                    ...
                    <p className="text-sm text-gray-500 mt-0.5">
                        Kelola data siswa & status akademik per angkatan
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
                    {angkatan.map((row) => {
                        const colors =
                            colorByTingkat[row.tingkat] ?? colorByTingkat["10"];
                        ...
                        return (
                            ...
                            >
                                {/* Top color band */}
                                <div
                                    className={`h-2 bg-gradient-to-r ${colors.bg}`}
                                />
                                ...
                                <div className="p-5 lg:p-6">
                                    <div className="flex items-start justify-between mb-4">
                                        ...
                                        <span
                                            className={`text-[10px] font-bold tracking-widest ${colors.chip} px-2 py-1 rounded-full uppercase`}
                                        >
                                            Tingkat {row.tingkat}
                                        </span>
                                    </div>
                                    ...
                                    <p className="text-xs text-gray-500 mb-5">
                                        {empty
                                            ...
                                            : "Klik untuk kelola siswa & variant"}
                                    </p>
                                    ...
                                    {row.variants.length > 0 && (
                                        <div className="mt-4 flex flex-wrap gap-1.5">
                                            {row.variants.map((v) => (
                                                <span
                                                    key={v}
                                                    className={`text-xs font-bold ${colors.chip} px-2 py-0.5 rounded`}
                                                >
                                                    {row.tingkat}
                                                    {v}
                                                </span>
                                    ...
                                    <div
                                        className={`mt-5 flex items-center text-sm font-semibold ${colors.text} group-hover:gap-2 transition-all`}
                                    >
                                        Buka data
