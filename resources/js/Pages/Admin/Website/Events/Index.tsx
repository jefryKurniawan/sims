                                ...
                                <tr key={e.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${
                                            e.is_Active === '0' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                                        }`}>
                                            {e.is_Active === '0' ? 'Aktif' : 'Tidak Aktif'}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                        <Link
                                            href={route('website.event.edit', e.id)}
                ...
                <div className="flex flex-wrap items-center justify-between px-4 py-2">
                    <div className="flex items-center flex-1">
                        <span className="text-sm text-gray-500">
                            Menampilkan {event.from} - {event.to} dari {event.total} data
                        </span>
                    </div>
                    <div className="flex items-center space-x-2">
                        ...
                        {event.links && event.links.map((link: any, index: number) => (
                            <a
                                key={index}
                                href={link.url}
