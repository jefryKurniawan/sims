        ...
        <AdminLayout
            ...
            header={
                <div className="flex items-center justify-between mb-4">
                    <div>
                        <h1 className="text-xl font-bold text-gray-800">Video</h1>
                    </div>
                    <Link
                                ...
                                <tr key={v.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${
                                            v.is_active === '0' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                                        }`}>
                                            {v.is_active === '0' ? 'Aktif' : 'Tidak Aktif'}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                        <Link
                                            href={route('website.video.edit', v.id)}
                ...
                <div className="flex flex-wrap items-center justify-between px-4 py-2">
                    <div className="flex items-center flex-1">
                        <span className="text-sm text-gray-500">
                            Menampilkan {video.from} - {video.to} dari {video.total} data
                        </span>
                    </div>
                    <div className="flex items-center space-x-2">
                        ...
                        {video.links && video.links.map((link: any, index: number) => (
                            <a
                                key={index}
                                href={link.url}
