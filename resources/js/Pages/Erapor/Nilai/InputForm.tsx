...
export default function InputForm({ jenis_nilai, siswas, mapels, tps }: Props) {
    ...
    const [tanggal, setTanggal] = React.useState(new Date().toISOString().split('T')[0]);
    ...
    const [batchValues, setBatchValues] = React.useState<Record<number, string>>({});
    const [batchCatatan, setBatchCatatan] = React.useState<Record<number, string>>({});
    const [isBatchMode, setIsBatchMode] = React.useState(false);

    const handleSingleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        form.post(isFormatif ? route('admin.erapor.nilai.store-formatif') : route('admin.erapor.nilai.store-sumatif'), {
    ...
    const handleBatchSubmit = (e: React.FormEvent) => {
        ...
        const nilaiArr: { rapor_siswa_id: number; nilai: string; catatan?: string }[] = [];
    ...
    return (
        <>
            <Head title={`Input Nilai ${isFormatif ? 'Formatif' : 'Sumatif'}`} />
            <div className="py-12">
                            ...
                            <div className="flex justify-between items-center">
                                <div>
                                    <h1 className="text-xl font-bold text-gray-900 dark:text-white">
                                        Input Nilai {isFormatif ? 'Asesmen Formatif' : 'Asesmen Sumatif'}
                                    </h1>
                                ...
                                <div className="flex gap-2">
                                    <Link
                                        href={route('admin.erapor.nilai.input', { jenis_nilai: 'formatif' })}
                                    ...
                                    <Link
                                        href={route('admin.erapor.nilai.input', { jenis_nilai: 'sumatif' })}
                                        className={`px-3 py-1.5 text-sm rounded ${
                                            !isFormatif
                                                ? 'bg-navy-600 text-white'
                                                : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300'
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow">
                        ...
                        <form onSubmit={handleBatchSubmit} className="p-6 space-y-6">
                                        ...
                                        >
                                            ...
                                            {tps.map((tp) => (
                                                <option key={tp.id} value={tp.id}>
                                                    {tp.kode_tp} - {tp.deskripsi.substring(0, 60)}
                                                </option>
                                            ))}
                                        </select>
                                    </div>
                            ...
                            {form.errors.rapor_mapel_id && (
                                <p className="text-red-500 text-xs">{form.errors.rapor_mapel_id}</p>
                            )}
                            {form.errors.tujuan_pembelajaran_id && (
                                <p className="text-red-500 text-xs">{form.errors.tujuan_pembelajaran_id}</p>
                            )}
                            ...
                            <div className="overflow-x-auto">
                                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                        {siswas.length === 0 && (
                                            <tr>
                                                <td colSpan={isFormatif ? 6 : 5} className="px-4 py-12 text-center text-gray-400">
                                        ...
                                        {siswas.map((siswa, idx) => (
                                            <tr key={siswa.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                                ...
                                                <td className="px-4 py-2 text-sm text-gray-500 dark:text-gray-400">{siswa.nisn}</td>
                                                ...
                                                {isFormatif && (
                                                    <td className="px-4 py-2 text-center">
                                                        <input
                                                            type="text"
                                                            value={batchCatatan[siswa.id] || ''}
                                                            onChange={(e) =>
                                                                setBatchCatatan((prev) => ({
                                                                    ...prev,
                                                                    [siswa.id]: e.target.value,
                                                                }))
                                                            }
