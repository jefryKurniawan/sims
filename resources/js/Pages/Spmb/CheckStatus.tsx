...
import { Search, AlertCircle, CheckCircle, XCircle, Clock, FileText, Mail, Calendar, User, ChevronRight } from 'lucide-react';
...
export default function CheckStatus({ applicant, error }: Props) {
    ...
    const statusConfig: Record<string, { label: string; color: string; bg: string; icon: any }> = {
        ...
        submitted: { label: 'Terkirim', color: 'text-blue-600', bg: 'bg-blue-100', icon: FileText },
        verified: { label: 'Terverifikasi', color: 'text-emerald-600', bg: 'bg-emerald-100', icon: CheckCircle },
    ...
    const getStatusBadge = (status: string | null) => {
        ...
        const cfg = statusConfig[status] || { label: status, color: 'text-gray-600', bg: 'bg-gray-100', icon: Clock };
        const Icon = cfg.icon;
        return (
            <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold ${cfg.color} ${cfg.bg}`}>
                    ...
                    <div className="max-w-2xl mx-auto">
                        <div className="text-center mb-8">
                            ...
                            <p className="text-gray-500">
                                Masukkan nomor registrasi untuk mengecek status pendaftaran Anda
                            </p>
                        </div>
                        ...
                        {applicant && (
                            <div ref={resultRef} className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                                ...
                                <div className="p-6 space-y-4">
                                    <div className="result-item flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                                        <div>
                                            ...
                                            <p className="font-bold text-gray-800 text-lg">{applicant.nomor_registrasi}</p>
                                        </div>
                                        {getStatusBadge(applicant.status_pendaftaran)}
                                    </div>
                                    ...
                                    <div className="result-item grid md:grid-cols-2 gap-4">
                                        ...
                                        <div className="flex items-start gap-3 p-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="font-medium text-gray-800">{jalurLabels[applicant.jalur_pendaftaran] || applicant.jalur_pendaftaran}</p>
                                            </div>
                                        </div>
                                    </div>

                                    <div className="result-item p-4 bg-emerald-50 rounded-xl">
                                        <p className="text-sm text-gray-500 mb-2">Asal Sekolah</p>
                                    ...
                                    {applicant.ranking && (
                                        <div className="result-item border border-gray-100 rounded-xl p-4">
                                            <h3 className="font-semibold text-gray-800 mb-3">Hasil Seleksi</h3>
                                            <div className="grid grid-cols-3 gap-4">
                                                <div className="text-center p-3 bg-gray-50 rounded-lg">
                                                    <p className="text-2xl font-bold text-emerald-600">{applicant.ranking.skor_total}</p>
                                                    <p className="text-xs text-gray-500">Skor Total</p>
                                                </div>
                                                <div className="text-center p-3 bg-gray-50 rounded-lg">
                                                    <p className="text-2xl font-bold text-primary">{applicant.ranking.peringkat}</p>
                                                    <p className="text-xs text-gray-500">Peringkat</p>
                                                </div>
                                                <div className="text-center p-3 bg-gray-50 rounded-lg">
                                                    {applicant.ranking.lulus_seleksi === true ? (
                                                        <CheckCircle className="w-8 h-8 text-emerald-500 mx-auto" />
                                                    ) : applicant.ranking.lulus_seleksi === false ? (
                                                        <XCircle className="w-8 h-8 text-red-500 mx-auto" />
                                                    ) : (
                                                        <Clock className="w-8 h-8 text-amber-500 mx-auto" />
                                                    )}
                                                    <p className="text-xs text-gray-500 mt-1">
                                                        {applicant.ranking.lulus_seleksi === true ? 'Lulus' :
                                                         applicant.ranking.lulus_seleksi === false ? 'Tidak Lulus' : 'Menunggu'}
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
